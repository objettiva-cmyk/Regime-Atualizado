# ============================================================
# MOTOR FISCAL RIR70 BY MC4
# Proprietario: MC4 CONTABILIDADE E GESTAO DE NEGOCIOS - CNPJ 09.944.432/0001-25
# Ferramenta proprietaria licenciada para uso interno do cliente autorizado.
# Proibida copia, distribuicao, sublicenciamento, remocao da identificacao MC4
# ou uso por terceiros sem autorizacao expressa da MC4.
# ============================================================
# -*- coding: utf-8 -*-
"""
...
* **F-02 (GAP-01) - PRECO_DO_PROPRIO_PRODUTO_EM_MES_ANTERIOR gera pendência em 04_PEND:**
  ...
  com NIVEL=REVISAR e TIPO_AJUSTE_SUGERIDO=APROVAR_CRITERIO_SUBSIDIARIO.
...
RUN_TS      = datetime.now()
# Atualização de versão do motor. Esta variável deve ser incrementada
# sempre que houver alterações significativas no comportamento ou na
# interface externa do script. A versão 1.3.12 consolida todas as
# melhorias de governança introduzidas até a data de 2026-05-21,
# incluindo obrigatoriedade de informar o período de execução,
# correções na pré‑validação (aliases normalizados para colunas de código/NCM, validação de ZIPs por período),
# padronização de códigos numéricos com zeros à esquerda, exclusão de ICMS‑ST do preço base,
# nomenclatura dinâmica de arquivos de ajustes e geração automática do mapa de similaridade.
SCRIPT_VER  = "1.3.28"
# A versão do critério de preço foi atualizada para refletir a nova
# interpretação de que o ICMS-ST (substituição tributária) não integra
# o valor do produto, nos mesmos moldes do IPI. Portanto, o valor
# utilizado como base para o cálculo do maior preço de venda será o
# preço bruto deduzido do IPI e do ICMS-ST. FCP-ST também é tratado
# como tributo destacável não cumulativo. Esta versão identifica a
# metodologia RIR70 sem ICMS-ST.
VERSAO_CRITERIO_PRECO = "RIR70_PRECO_BRUTO_SEM_ICMS_ST_v1"
...
DEFAULT_CONFIG = {
    ...
    "permitir_documentos_em_input_raiz": False,
    "permitir_documentos_em_movimento_item": False,
    "bloquear_situacao_documento_numerica_2": True,
    "permitir_movimentacao_em_input_raiz": False,
    "permitir_inventario_em_input_raiz": False,
    ...
    "permitir_fallback_base_inventario_competencia_anterior": False,
    "priorizar_base_competencia_anterior_mesmo_cnpj": True,
    "converter_inventario_para_unidade_tributavel": True,
    "permitir_fallback_base_inventario_entre_cnpjs": False,
    "destacar_fallback_base_inventario_entre_cnpjs": True,
    "permitir_fallback_base_produto_similar": False,
    "arquivo_mapa_similaridade_produto": "MAPA_SIMILARIDADE_PRODUTO.xlsx",
    "regex_arquivo_inventario": r"^Inventario_(\d{14})_((0[1-9]|1[0-2])\d{4})\.xlsx$",
    "bloquear_arquivo_inventario_fora_padrao": True,
    "bloquear_inventario_duplicado_mes_cnpj": True,
    "bloquear_competencia_inventario_ausente": True,
    "usar_inventario_mes_anterior_como_saldo_inicial": False,
    "competencia_saldo_inicial": "",
    "bloquear_pre_validacao": True,
    "validar_pasta_input_limpa": True,
    "bloquear_arquivo_lixo_input": True,
    ...
    "fonte_excel": "Aptos Narrow",
    "tamanho_fonte_excel": 10,
    "aplicar_layout_premium_output": "Sim",
    "aplicar_layout_premium_pre_validacao": "Sim",
    "aplicar_layout_premium_ajustes": "Sim",
    "layout_usar_arquivo_raw_temporario": "Nao",
    "layout_manter_raw_temporario": "Nao",
    "layout_bloquear_se_falhar": "Nao",
    "layout_modo_rapido_acima_linhas": 10000,
    "layout_max_largura_coluna": 42,
    "layout_remover_colunas_vazias": "Nao",
    "layout_ordenar_chave_nfe_pre_validacao": "Sim",
    "layout_ordenar_chave_nfe_movimento": "Nao",
    "layout_nivel": "executivo",
    "layout_aplicar_zebra": "Nao",
    "layout_validacao_profunda": "Nao",
    "layout_preservar_raw_em_auditoria": "Sim",
    ...
    ,
    # Ativa ou não a interrupção da execução caso a pré‑validação identifique
    # inconsistências nos arquivos de entrada. Quando False (padrão), a
    # pré‑validação apenas gera um relatório de inconsistências e prossegue.
    "bloquear_pre_validacao": False
}
...
# ─── Layout premium MC4 desacoplado do núcleo fiscal ─────────────────────────
def _layout_config_mc4(config):
    """Parâmetros visuais isolados do cálculo fiscal."""
    config = config or {}
    return {
        "modo_rapido_acima_linhas": config.get("layout_modo_rapido_acima_linhas", 50000),
        "max_largura_coluna": config.get("layout_max_largura_coluna", 42),
        "remover_colunas_vazias": "Nao",
        "ordenar_chave_nfe_pre_validacao": config.get("layout_ordenar_chave_nfe_pre_validacao", "Sim"),
        "ordenar_chave_nfe_movimento": config.get("layout_ordenar_chave_nfe_movimento", "Nao"),
        "fonte_corpo": config.get("fonte_excel", "Aptos Narrow"),
        "fonte_cabecalho": config.get("fonte_excel", "Aptos Narrow"),
        "tamanho_corpo": int(config.get("tamanho_fonte_excel", 10) or 10),
        "tamanho_cabecalho": int(config.get("tamanho_fonte_excel", 10) or 10),
        "usar_cabecalhos_amigaveis": "Sim",
        "limpar_estilos_residuais_apos_dados": "Sim",
        "layout_nivel": config.get("layout_nivel", "rapido"),
        "aplicar_zebra": config.get("layout_aplicar_zebra", "Nao"),
        "validacao_profunda": config.get("layout_validacao_profunda", "Nao"),
    }


def aplicar_layout_premium_mc4_se_configurado(input_path, output_path=None, config=None, perfil="output_fiscal"):
    """Aplica o layout final somente depois que o Excel técnico já foi gerado.

    O motor permanece responsável por regras, cálculos, parâmetros e rastreabilidade.
    O layout fica restrito a apresentação visual.
    """
    cfg = config or {}
    chave = {
        "output_fiscal": "aplicar_layout_premium_output",
        "pre_validacao": "aplicar_layout_premium_pre_validacao",
        "ajustes_analista": "aplicar_layout_premium_ajustes",
    }.get(perfil, "aplicar_layout_premium_output")
    if not _is_sim(cfg.get(chave, "Sim")):
        return {"layout_mc4": "NAO_APLICADO", "perfil": perfil, "motivo": f"{chave}=Nao"}

    try:
        from layout_premium_mc4 import aplicar_layout_premium
        result = aplicar_layout_premium(input_path, output_path or input_path, perfil=perfil, config=_layout_config_mc4(cfg))
        cfg[f"_layout_{perfil}"] = result
        try:
            logger.info("Layout premium MC4 aplicado (%s): %s", perfil, safe_json_dumps(result))
        except Exception:
            pass
        return result
    except Exception as exc:
        result = {"layout_mc4": "ERRO", "perfil": perfil, "erro": str(exc)}
        cfg[f"_layout_{perfil}"] = result
        try:
            logger.exception("Falha ao aplicar layout premium MC4 (%s): %s", perfil, exc)
        except Exception:
            pass
        if _is_sim(cfg.get("layout_bloquear_se_falhar", "Sim")):
            raise ConfigError(f"Falha ao aplicar layout premium MC4 ({perfil}): {exc}")
        print(f"Aviso: falha ao aplicar layout premium MC4 ({perfil}): {exc}")
        return result



# ─── Pré‑validação de arquivos de entrada ─────────────────────────────────────
def _pre_validacao(config) -> None:
    """
    Executa validações básicas nos arquivos de entrada antes do cálculo principal.

    Esta rotina percorre as pastas de movimento e inventário definidas no
    ``config``, calcula um hash SHA‑256 para cada arquivo encontrado e tenta
    abrir arquivos Excel (``.xlsx``) com o pandas para verificar a presença de
    colunas essenciais (código do item e NCM). Caso encontre problemas de
    leitura ou colunas ausentes, registra estas ocorrências em uma planilha
    denominada ``00_PRE_VALIDACAO.xlsx`` no diretório de saída. Essa planilha
    contém as seguintes colunas: ``arquivo``, ``sha256``, ``data_modificacao`` e
    ``issue``.

    A pré‑validação não bloqueia automaticamente a execução do motor; ela apenas
    alerta o usuário sobre inconsistências. Para interromper o processamento
    quando houver qualquer issue, defina no ``config`` a chave
    ``bloquear_pre_validacao`` como ``True``.
    """
    try:
        from pandas import ExcelFile  # type: ignore
    except Exception:
        logger.warning("Pandas não está disponível para pré‑validação.")
        return

    dirs = []
    for key in ["pasta_movimentacao", "pasta_inventario"]:
        p = clean_str(config.get(key)) or ""
        if not p:
            continue
        # Converte para caminho absoluto relativo ao BASE_DIR
        dir_path = BASE_DIR / p if not Path(p).is_absolute() else Path(p)
    ...
    for d in dirs:
        ...
        for entry in d.iterdir():
            if not entry.is_file():
                continue
            # Ignora arquivos temporarios do Excel e aplica a mesma trava de periodo
            # usada na importacao principal. Arquivos com intervalo no nome, como
            # "de 01-01-2025 a 30-06-2025", somente entram na pre-validacao se
            # sobrepuserem o periodo selecionado na execucao.
            if entry.name.startswith("~$"):
                continue
            if not _file_overlaps_period(entry, config):
                continue
            try:
            ...
            if entry.suffix.lower() == ".xlsx":
                # Tenta abrir e verificar colunas minimas. A validacao varre as
                # primeiras linhas para localizar cabecalho real, pois alguns
                # exports DFE nao usam a primeira linha como header.
                try:
                    with pd.ExcelFile(entry) as xl:
                        sheet = xl.sheet_names[0] if xl.sheet_names else None
                        if sheet:
                            preview = pd.read_excel(entry, sheet_name=sheet, header=None, nrows=HEADER_SCAN, dtype=object, keep_default_na=False)
                            is_inventory = "inventario" in str(entry).lower() or "estoque" in entry.name.lower() or "saldo" in entry.name.lower()
                            if is_inventory:
                                best = find_inventory_header(preview)
                                headers = make_unique_headers(preview.iloc[best.get("idx",0)].tolist()) if len(preview) else []
                                has_codigo = pick_inventory_column(headers, "codigo_item") is not None
                                has_qtd = pick_inventory_column(headers, "qtde_inventario") is not None
                                if not has_codigo or not has_qtd:
                                    issue_msg = "Colunas obrigatorias ausentes no inventario (CODIGO ou QUANTIDADE)."
                            else:
                                best = find_header_row(preview, config.get("mapeamento_colunas") or {})
                                headers = make_unique_headers(preview.iloc[best.get("idx",0)].tolist()) if len(preview) else []
                                has_codigo = pick_column(headers, "codigo_item", config.get("mapeamento_colunas") or {}) is not None
                                has_ncm = pick_column(headers, "ncm", config.get("mapeamento_colunas") or {}) is not None
                                if not has_codigo or not has_ncm:
                                    issue_msg = "Colunas obrigatorias ausentes no movimento (CODIGO ou NCM)."
                except Exception as exc:
                    issue_msg = f"Falha ao abrir: {exc}"
            # Apenas registra issues quando houver mensagem
            if issue_msg:
                issues.append({
                    "arquivo": str(entry.relative_to(BASE_DIR)) if BASE_DIR in entry.parents else str(entry),
                    "sha256": sha256,
    ...
    if issues:
        try:
            pre_path = OUTPUT_DIR / "00_PRE_VALIDACAO.xlsx"
            df_issues = pd.DataFrame(issues)
            with pd.ExcelWriter(pre_path) as writer:
                df_issues.to_excel(writer, sheet_name="INCONSISTENCIAS", index=False)
            aplicar_layout_premium_mc4_se_configurado(pre_path, pre_path, config, perfil="pre_validacao")
            logger.warning("Pré‑validação detectou %d problemas. Relatório: %s", len(issues), pre_path)
            print(f"Aviso: pré‑validação detectou {len(issues)} problemas. Consulte {pre_path}.")
            # Interrompe se configurado para bloquear em caso de issues
            if _is_sim(config.get("bloquear_pre_validacao", False)):
                raise ConfigError(f"Pré‑validação bloqueada devido a {len(issues)} inconsistências.")
        except Exception as exc:
            logger.error("Falha ao salvar relatório de pré‑validação: %s", exc)

    else:
        logger.info("Pré‑validação concluída. Nenhuma inconsistência encontrada.")


# ─── Geração automática do mapa de similaridade ──────────────────────────────
def _gerar_mapa_automatico(config) -> None:
    """
    Regenera o arquivo ``MAPA_SIMILARIDADE_PRODUTO.xlsx`` com base na
    ``MATRIZ_CADASTRO_PRODUTO_RIR70_SIMILARIDADE.xlsx`` caso a matriz tenha
    sido atualizada ou o mapa não exista.

    A função busca a matriz na pasta auxiliar configurada e verifica se o
    arquivo do mapa está desatualizado comparando datas de modificação. Se o
    mapa estiver ausente ou mais antigo que a matriz, ele será recriado.
    Somente produtos com ``STATUS_CADASTRO = ATIVO``, ``PART_ARBITRAMENTO = S``
    (ou ``PARTICIPA_ARBITR = S``) e ``PERMITE_FALLBACK_SIMILAR = S`` são
    incluídos. As colunas principais são copiadas da matriz e um resumo por
    grupo de similaridade é gerado na segunda aba. Se algum erro ocorrer,
    registra no log e ignora silenciosamente.
    """
    try:
        aux_dir = clean_str(config.get("pasta_auxiliares", "input/auxiliares"))
        aux_dir_path = Path(aux_dir) if Path(aux_dir).is_absolute() else BASE_DIR / aux_dir
        # Determina caminho da matriz
        candidatos = [
            aux_dir_path / clean_str(config.get("arquivo_matriz_cadastro_produto", "MATRIZ_CADASTRO_PRODUTO_RIR70_SIMILARIDADE.xlsx")),
            aux_dir_path / "MATRIZ_CADASTRO_PRODUTO_RIR70_SIMILARIDADE.xlsx",
            BASE_DIR / "MATRIZ_CADASTRO_PRODUTO_RIR70_SIMILARIDADE.xlsx"
        ]
        matriz_file = next((p for p in candidatos if p.exists()), None)
        if not matriz_file:
            return
        mapa_path = aux_dir_path / clean_str(config.get("arquivo_mapa_similaridade_produto", "MAPA_SIMILARIDADE_PRODUTO.xlsx"))
        # Garante que a pasta de auxiliares exista antes de gravar o arquivo
        try:
            aux_dir_path.mkdir(parents=True, exist_ok=True)
        except Exception:
            pass
        # Verifica se mapa precisa ser regenerado
        try:
            if mapa_path.exists() and matriz_file.stat().st_mtime <= mapa_path.stat().st_mtime:
                return
        except Exception:
            pass
        # Lê matriz e filtra registros relevantes
        try:
            df = pd.read_excel(matriz_file, sheet_name="MATRIZ_CADASTRO", dtype=object, keep_default_na=False)
        except Exception:
            logger.warning("Não foi possível ler a planilha MATRIZ_CADASTRO para gerar o mapa.")
            return
        ...
        for _, row in df.iterrows():
            status_cad = clean_str(get(row, "STATUS_CADASTRO")).upper()
            part = clean_str(get(row, "PART_ARBITRAMENTO") or get(row, "PARTICIPA_ARBITR")).upper()
            permite = clean_str(get(row, "PERMITE_FALLBACK_SIMILAR")).upper()
            if status_cad != "ATIVO":
                continue
            if part not in {"S", "SIM", "Y", "YES"}:
                continue
            if permite not in {"S", "SIM", "Y", "YES"}:
                continue
            rows.append({
                "CODIGO": get(row, "CODIGO"),
                "CODIGO_BASE": get(row, "CODIGO_BASE"),
                "DESCRICAO_TECNICA": get(row, "DESCRICAO_TECNICA") or get(row, "DESCRICAO_PRODUTO") or get(row, "DESCRICAO"),
                "TIPO_PRODUTO": clean_str(get(row, "TIPO_PRODUTO")).upper(),
                "STATUS_CADASTRO": status_cad,
                "GRUPO_SIMILARIDADE_HOMOLOGADO": get(row, "GRUPO_SIMILARIDADE_HOMOLOGADO"),
                "STATUS_HOMOLOGACAO": clean_str(get(row, "STATUS_HOMOLOGACAO")).upper(),
                "PERMITE_FALLBACK_SIMILAR": permite,
                "PART_ARBITRAMENTO": part,
                "NCM": normalize_ncm(get(row, "NCM")),
                "CEST": normalize_cest(get(row, "CEST")),
                "MARCA_INTERNA": get(row, "MARCA_INTERNA"),
                "EMBALAGEM": get(row, "EMBALAGEM"),
                "VOLUME_ML": get(row, "VOLUME_ML"),
            })
        if not rows:
            logger.warning("Nenhuma linha elegível para o mapa de similaridade.")
            return
        map_df = pd.DataFrame(rows)
        # Resumo por grupo de similaridade
        try:
            resumo = map_df.groupby("GRUPO_SIMILARIDADE_HOMOLOGADO").agg(NUM_CODIGOS=("CODIGO", "count")).reset_index()
        except Exception:
            resumo = pd.DataFrame()
        # Escreve arquivo
        try:
            with pd.ExcelWriter(mapa_path) as writer:
                map_df.to_excel(writer, sheet_name="MAPA_SIMILARIDADE", index=False)
                if not resumo.empty:
                    resumo.to_excel(writer, sheet_name="RESUMO_MAPA", index=False)
            logger.info("Mapa de similaridade gerado automaticamente: %s", mapa_path)
        except Exception as exc:
            logger.error("Erro ao gravar mapa de similaridade: %s", exc)
    except Exception:
        # Qualquer erro silencioso para não interromper o motor
        logger.exception("Falha inesperada na geração automática do mapa")
        return
...
def normalize_code_text(v) -> str:
    """Normaliza o código do item, removendo sufixos ".0" gerados por planilhas
    e garantindo que códigos puramente numéricos tenham pelo menos 6 caracteres.

    Exemplos:
    * "123456.0" -> "123456"
    * "52124"   -> "052124" (zero à esquerda para completar 6 dígitos)
    * "ABC123"  -> "ABC123" (mantém alfanumérico sem alterações)
    """
    t = clean_str(v)
    # Se estiver no formato "12345.0" gerado por Excel, remove a parte decimal
    if re.fullmatch(r"\d+\.0", t):
        t = t[:-2]
    # Se o código for puramente numérico, preenche zeros à esquerda até atingir 6 dígitos
    if re.fullmatch(r"\d+", t):
        # Não altera códigos já maiores ou iguais a 6 caracteres
        return t.zfill(6) if len(t) < 6 else t
    return t
...
def parse_competencia(v, data_emissao, config=None):
    ...
    estiver ausente ou não puder ser convertido, o critério subsidiário para a data
    de emissão somente ocorrerá se
    ``permitir_fallback_competencia_data_emissao`` for verdadeiro.  Sem
    critério subsidiário, será retornado "N/D".
    """
...
INVENTARIO_ARQUIVO_REGEX_DEFAULT = r"^Inventario_(\d{14})_((0[1-9]|1[0-2])\d{4})\.xlsx$"

def _inventario_regex(config):
    return clean_str((config or {}).get("regex_arquivo_inventario")) or INVENTARIO_ARQUIVO_REGEX_DEFAULT

def parse_inventory_filename(file_path, config=None):
    """Retorna metadados CNPJ/competencia do arquivo Inventario_CNPJ_MMAAAA.xlsx."""
    name = Path(file_path).name
    m = re.match(_inventario_regex(config or DEFAULT_CONFIG), name, flags=re.I)
    if not m:
        return None
    cnpj = normalize_cnpj(m.group(1))
    comp = m.group(2)
    return {"cnpj": cnpj, "competencia_mmaaaa": comp, "competencia": f"{comp[:2]}/{comp[2:]}", "mes": comp[:2], "ano": comp[2:]}

def _inventory_file_matches_period(file_path, config):
    meta = parse_inventory_filename(file_path, config)
    if not meta:
        return False
    ...
    if not inicio or not fim:
        return True
    comp_dt = datetime(int(meta["ano"]), int(meta["mes"]), 1)
    return datetime(inicio.year, inicio.month, 1) <= comp_dt <= datetime(fim.year, fim.month, 1)

def _competencias_periodo_mmaaaa(config):
    inicio = parse_config_date(config.get("periodo_base_inicio"))
    fim = parse_config_date(config.get("periodo_base_fim"))
    if not inicio or not fim:
        return []
    return [c.replace("/", "") for c in _month_range(inicio, fim)]

def _scan_inventory_folder_producao(config):
    folder = Path(config.get("pasta_inventario", "input/inventario"))
    if not folder.is_absolute():
        folder = BASE_DIR / folder
    folder.mkdir(parents=True, exist_ok=True)
    invalidos, files, dup = [], [], []
    ...
    for p in sorted(folder.iterdir()):
        if p.is_dir():
            invalidos.append((p.name, "SUBPASTA_NAO_PERMITIDA")); continue
        if p.name.startswith("~$"):
            invalidos.append((p.name, "TEMPORARIO_EXCEL")); continue
        meta = parse_inventory_filename(p, config) if p.suffix.lower() in allowed_suffixes else None
        if not meta:
            invalidos.append((p.name, "NOME_FORA_PADRAO_INVENTARIO_CNPJ_MMAAAA")); continue
        key = (meta["cnpj"], meta["competencia_mmaaaa"])
        if key in seen:
            dup.append((p.name, seen[key].name, key[0], key[1]))
        else:
            seen[key] = p
        files.append(p)
    return folder, files, invalidos, dup

def discover_inventory_files(config):
    if clean_str(config.get("inventario_formato_competencia")).lower() == "arquivos_mensais_por_cnpj":
        folder, files, invalidos, dup = _scan_inventory_folder_producao(config)
        if invalidos and _is_sim(config.get("bloquear_arquivo_inventario_fora_padrao", True)):
            detalhes = "; ".join([f"{n}:{m}" for n, m in invalidos[:10]])
            raise ConfigError("Arquivos invalidos em input/inventario. Remova lixo/placeholders/copias. " + detalhes)
        if dup and _is_sim(config.get("bloquear_inventario_duplicado_mes_cnpj", True)):
            detalhes = "; ".join([f"{n} duplica {o} ({c}/{comp})" for n, o, c, comp in dup[:10]])
            raise ConfigError("Inventario duplicado por CNPJ+competencia. " + detalhes)
        out = [p for p in files if _inventory_file_matches_period(p, config)]
        if not out and _is_sim(config.get("bloquear_competencia_inventario_ausente", True)):
            comps = ",".join(_competencias_periodo_mmaaaa(config)) or "periodo_nao_definido"
            raise ConfigError(f"Nenhum arquivo de inventario encontrado para a competencia do periodo ({comps}) em {folder}.")
        return sorted(out)
    files=[]
    if config.get("usar_subpastas_input",True):
...
def discover_xml_files(config):
    """Descobre XML/ZIP de NF-e de forma deterministica e auditavel.

    A versao 1.3.12 amplia a busca para as pastas operacionais mais comuns
    sem criar segunda fonte de verdade: todo arquivo encontrado continua sendo
    registrado em FONTES_PROCESSADAS com codigo de controle.
    """
    if not config.get("usar_xml_como_movimentacao", True):
        return []
    folder_candidates = [
        BASE_DIR / clean_str(config.get("pasta_xml", "input/xml")),
        BASE_DIR / "input" / "XML",
        BASE_DIR / clean_str(config.get("pasta_movimentacao", "input/movimento_item")),
        INPUT_DIR,
    ]
    files = []
    ...
    seen = set(); out = []
    for p in sorted(files, key=lambda x: str(x.resolve()).lower()):
        rp = str(p.resolve())
        if rp not in seen:
            seen.add(rp); out.append(p)
    return out
...
def _legacy_build_record_rir70(row,mapping,config,manual_factors,ipi_column_present,row_origin=""):
    ...
    # A competência deve respeitar as configurações de prioridade e critério subsidiário definidas no config.
...
def _xml_rows_from_bytes(xml_bytes, source="XML"):
    """Lê XML de NF-e e retorna status fiscal/técnico explícito.

    Não mascara XML malformado, XML sem itens ou XML sem protocolo.
    Esses estados aparecem na pré-validação e em FONTES_PROCESSADAS.
    """
    try:
        root = ET.fromstring(xml_bytes)
    except Exception as e:
        return [], f"XML_MALFORMADO: {source}: {e}"
...
def _xml_rows_from_zip(zip_path):
    ...
    try:
        with zipfile.ZipFile(zip_path) as zf:
            xml_names = [n for n in sorted(zf.namelist()) if n.lower().endswith(".xml")]
            if not xml_names:
            ...
            for name in xml_names:
                ...
                rows, status = _xml_rows_from_bytes(data, f"{Path(zip_path).name}|{name}")
                statuses.append(f"{name}:{status}")
                if status.startswith("XML_MALFORMADO") or status.startswith("XML_SEM_ITENS"):
                    continue
                all_rows.extend(rows)
        RIR70_SOURCE_CONTROL_CODES[Path(zip_path).name] = _sha256_bytes("\n".join(inner_hashes).encode("utf-8")) if inner_hashes else sha256_file(zip_path)
    except Exception as e:
        return [], f"Falha ZIP XML: {e}"
    hard_errors = [st for st in statuses if ":XML_MALFORMADO" in st or ":XML_SEM_ITENS" in st]
    if hard_errors:
        return all_rows, "ZIP_XML_COM_ERROS: " + " | ".join(hard_errors[:10])
    if not all_rows:
        return [], "ZIP_XML_SEM_LINHAS_IMPORTAVEIS: " + " | ".join(statuses[:10])
    non_ok = [st for st in statuses if not st.endswith(":OK")]
    if non_ok:
        return all_rows, "ZIP_XML_COM_ALERTAS: " + " | ".join(non_ok[:10])
    return all_rows, "OK"
...
def load_xml_inputs(config, manual_factors, periodo_inicio, periodo_fim, fingerprints, records, duplicates, logs):
    ...
    for file_path, rows, status, sha in parsed_results:
        ...
        if status != "OK":
            status_tag = "ERRO_XML" if ("MALFORMADO" in status or "SEM_ITENS" in status or status.startswith("Falha")) else "ALERTA_XML"
            logs.append((file_path.name, "XML", status_tag, status))
            if status_tag == "ERRO_XML" and not rows:
                continue
        before = len(records)
        out = dup = empty = 0
...
def load_precos_referencia(config):
    """Carrega tabela de preços de referência manuais como último critério subsidiário auditável.
    ...
    Regras de produção:
      ...
      - nunca substitui preço real de venda já localizado: é aplicado apenas quando todos os critério subsidiários falham.
    """
...
def _write_inventario_pivot(writer, wb, periodo_txt, inv_rows, fmts):
    ...
    for ri, prod in enumerate(rows, start=4):
        ...
        for n, comp in enumerate(comps):
            ...
            if extras:
                status_txt = f"{status}\n" + " | ".join(extras)
            fmt_status = _bad if "SEM BASE" in status_txt else (_warn if "OUTRO CNPJ" in status_txt or "CRITERIO_SUBSIDIARIO" in origem or "ANTERIOR" in status_txt else _ok)
...
# =============================================================================
# MC4 - IDENTIFICACAO PROPRIETARIA, LICENCA E INTEGRIDADE DO PACOTE
# =============================================================================
...
MOTOR_LICENSED_TO_DEFAULT = "101 BRASIL INDUSTRIA DE BEBIDAS LTDA"
MOTOR_LICENSE_TYPE = "Uso interno, nao exclusivo, intransferivel"
MC4_BUILD_ID = "v1.3.28-producao-forense"
MC4_REQUIRED_MARKERS = [
    "MC4 CONTABILIDADE",
    "Motor Fiscal RIR70 by MC4",
    "Ferramenta proprietaria",
    "Uso interno"
]
MC4_PUBLIC_KEY_PEM = '-----BEGIN PUBLIC KEY-----\nMCowBQYDK2VwAyEA3oPBDouCzZW59j1z4oiMZv9kYvZTpEBIwMXX7xmNwWc=\n-----END PUBLIC KEY-----\n'
...
def _mc4_manifest_paths():
    base = _mc4_base_dir()
    return base / "MANIFESTO_PACOTE_MC4.json", base / "MANIFESTO_PACOTE_MC4.sig"


def _mc4_read_bytes(path):
    try:
...
def _mc4_verify_signature(manifest_bytes, sig_bytes):
    if not manifest_bytes or not sig_bytes:
        return False, "Manifesto ou assinatura ausente"
    try:
        import base64 as _base64
        from cryptography.hazmat.primitives import serialization as _serialization

        pub_path = _mc4_base_dir() / "MC4_PUBLIC_KEY.pem"
        if pub_path.exists():
            pub_pem = pub_path.read_bytes()
        else:
            pub_pem = MC4_PUBLIC_KEY_PEM.encode("ascii")

        sig_raw = sig_bytes.strip()
        try:
            sig_decoded = _base64.b64decode(sig_raw, validate=True)
        except Exception:
            sig_decoded = sig_raw

        pub = _serialization.load_pem_public_key(pub_pem)
        pub.verify(sig_decoded, manifest_bytes)
        return True, "Assinatura valida"
    except Exception as exc:
        return False, f"Assinatura invalida: {exc}"

def validar_integridade_mc4(config):
    """Valida autoria MC4, manifesto assinado e hashes dos arquivos criticos."""
    cfg = config or {}
    lic = cfg.get("licenca_mc4") or {}
    modo = norm_text(lic.get("modo_licenca") or cfg.get("MODO_EXECUCAO") or "Producao")
    bloquear = _is_sim(lic.get("bloquear_se_integridade_violada", True)) and modo in {"PRODUCAO", "PRODUCAOLICENCIADA"}
    manifest_path, sig_path = _mc4_manifest_paths()
    manifest_bytes = _mc4_read_bytes(manifest_path)
    sig_bytes = _mc4_read_bytes(sig_path)
    ok_sig, msg_sig = _mc4_verify_signature(manifest_bytes, sig_bytes)
    violacoes = []
    arquivos = []
    manifest = {}
    if not manifest_bytes:
        violacoes.append("MANIFESTO_MC4_AUSENTE")
    else:
        try:
            manifest = json.loads(manifest_bytes.decode("utf-8"))
        except Exception as exc:
            violacoes.append(f"MANIFESTO_MC4_INVALIDO: {exc}")
    if not ok_sig:
        violacoes.append(msg_sig)
    base = _mc4_base_dir()
    for item in manifest.get("arquivos", []) if isinstance(manifest, dict) else []:
        rel = clean_str(item.get("nome"))
        esperado = clean_str(item.get("sha256"))
        if not rel:
            continue
        p = base / rel
        encontrado = sha256_file(p)
        status = "OK" if esperado and encontrado == esperado else "DIVERGENTE"
        if status != "OK":
            violacoes.append(f"HASH_DIVERGENTE: {rel}")
        arquivos.append({"arquivo": rel, "sha256_esperado": esperado, "sha256_encontrado": encontrado, "status": status})
    # Verificacao textual da marca nos arquivos principais. ASCII friendly.
    for rel in ["motor_arbitramento.py", "Executar_Motor_RIR70_MC4.bat", "Abrir_Sistema_RIR70_MC4.bat", "sistema_rir70_web.py", "LICENCA_MC4.txt"]:
        p = base / rel
        txt = ""
        try:
            ...
            pass
        if p.exists():
            for marker in ["MC4", "Motor Fiscal RIR70 by MC4"]:
                if marker not in txt:
                    violacoes.append(f"IDENTIFICACAO_PROPRIETARIA_REMOVIDA: {rel} sem {marker}")
                    break
    status = "OK" if not violacoes else "VIOLADA"
    result = {
        "status_integridade": status,
        "assinatura_manifesto": "VALIDA" if ok_sig else "INVALIDA",
        "mensagem_assinatura": msg_sig,
        "manifesto": str(manifest_path),
        "arquivos": arquivos,
        "violacoes": violacoes,
        "proprietario": MOTOR_OWNER,
        "proprietario_cnpj": MOTOR_OWNER_CNPJ,
        "produto": MOTOR_BRAND,
        "cliente_licenciado": lic.get("cliente_licenciado") or MOTOR_LICENSED_TO_DEFAULT,
        "modo_licenca": lic.get("modo_licenca", cfg.get("MODO_EXECUCAO", "Producao")),
    }
    cfg["_integridade_mc4"] = result
    if violacoes:
        # Registra arquivo de violacao mesmo antes do logging estar inicializado.
        try:
            out_dir = _mc4_base_dir() / "logs_integridade"
            out_dir.mkdir(exist_ok=True)
            vp = out_dir / ("VIOLACAO_INTEGRIDADE_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".json")
            vp.write_text(safe_json_dumps(result, indent=2), encoding="utf-8")
        except Exception:
            pass
        if bloquear:
            raise ConfigError("Integridade MC4 violada. Execucao bloqueada em modo Producao. Violacoes: " + "; ".join(violacoes[:5]))
    return cfg


def validar_cnpjs_licenciados_mc4(records, config):
    """Valida se os CNPJs encontrados constam na licenca MC4."""
    cfg = config or {}
    lic = cfg.get("licenca_mc4") or {}
    autorizados = {normalize_cnpj(x) for x in (lic.get("cnpjs_autorizados") or []) if normalize_cnpj(x)}
    if not autorizados:
        return []
    encontrados = set()
    for r in records or []:
        for k in ["CNPJ Emitente", "EMPRESA"]:
            c = normalize_cnpj(r.get(k)) if isinstance(r, dict) else ""
            if c:
                encontrados.add(c)
    fora = sorted(c for c in encontrados if c not in autorizados)
    cfg.setdefault("_licenca_mc4_cnpjs_nao_autorizados", fora)
    modo = norm_text(lic.get("modo_licenca") or cfg.get("MODO_EXECUCAO") or "Producao")
    bloquear = _is_sim(lic.get("bloquear_cnpj_nao_licenciado", False)) and modo in {"PRODUCAO", "PRODUCAOLICENCIADA"}
    if fora and bloquear:
        raise ConfigError("CNPJ nao licenciado pela MC4 em modo Producao: " + ", ".join(fora[:10]))
    return fora


def _mc4_license_capa_rows(config):
    cfg = config or {}
    lic = cfg.get("licenca_mc4") or {}
    integ = cfg.get("_integridade_mc4") or {}
    fora = cfg.get("_licenca_mc4_cnpjs_nao_autorizados") or []
    viol = integ.get("violacoes") or []
    return [
        {"INDICADOR":"MC4_PRODUTO", "VALOR":lic.get("produto") or MOTOR_BRAND},
        {"INDICADOR":"MC4_PROPRIETARIO", "VALOR":lic.get("proprietario") or MOTOR_OWNER},
        {"INDICADOR":"MC4_CNPJ_PROPRIETARIO", "VALOR":lic.get("proprietario_cnpj") or MOTOR_OWNER_CNPJ},
        {"INDICADOR":"MC4_CLIENTE_LICENCIADO", "VALOR":lic.get("cliente_licenciado") or MOTOR_LICENSED_TO_DEFAULT},
        {"INDICADOR":"MC4_TIPO_LICENCA", "VALOR":lic.get("tipo_licenca") or MOTOR_LICENSE_TYPE},
        {"INDICADOR":"MC4_USO_AUTORIZADO", "VALOR":lic.get("uso_autorizado") or "Uso interno do cliente licenciado"},
        {"INDICADOR":"MC4_PROPRIEDADE_INTELECTUAL", "VALOR":"Framework, scripts, layout, metodologia, validacoes, templates e fluxos de auditoria pertencem a MC4. Dados fiscais e documentos processados pertencem ao cliente licenciado."},
        {"INDICADOR":"MC4_STATUS_INTEGRIDADE", "VALOR":integ.get("status_integridade", "NAO VALIDADA")},
        {"INDICADOR":"MC4_ASSINATURA_MANIFESTO", "VALOR":integ.get("assinatura_manifesto", "NAO VALIDADA")},
        {"INDICADOR":"MC4_VIOLACOES_INTEGRIDADE", "VALOR":"; ".join(viol) if viol else "Nenhuma violacao detectada"},
        {"INDICADOR":"MC4_CNPJS_NAO_LICENCIADOS", "VALOR":", ".join(fora) if fora else "Nenhum"},
    ]


def _mc4_print_banner():
    print("============================================================")
    print("MC4 CONTABILIDADE E GESTAO DE NEGOCIOS")
    print("CNPJ 09.944.432/0001-25")
    print("Motor Fiscal RIR70 by MC4 - Ferramenta proprietaria")
    print("Uso interno do cliente licenciado")
    print("============================================================")
...
def ensure_config():
    ...
    cfg.setdefault("licenca_mc4", {})
    if not isinstance(cfg.get("licenca_mc4"), dict):
        cfg["licenca_mc4"] = {}
    lic = cfg["licenca_mc4"]
    lic.setdefault("proprietario", MOTOR_OWNER)
    lic.setdefault("proprietario_cnpj", MOTOR_OWNER_CNPJ)
    lic.setdefault("produto", MOTOR_BRAND)
    lic.setdefault("cliente_licenciado", MOTOR_LICENSED_TO_DEFAULT)
    lic.setdefault("tipo_licenca", MOTOR_LICENSE_TYPE)
    lic.setdefault("uso_autorizado", "Uso interno do cliente licenciado")
    lic.setdefault("modo_licenca", "Producao")
    lic.setdefault("bloquear_se_integridade_violada", True)
    lic.setdefault("bloquear_cnpj_nao_licenciado", True)
    lic.setdefault("permitir_execucao_adulterada", False)
    lic.setdefault("restricoes", [
        "Proibida distribuicao a terceiros",
        "Proibido sublicenciamento",
        "Proibida remocao da identificacao MC4",
        "Proibido uso para empresas nao autorizadas sem nova licenca"
    ])


    cfg.setdefault("MODO_EXECUCAO", "Producao")
    modo_exec_norm = norm_text(cfg.get("MODO_EXECUCAO"))
    if modo_exec_norm != "PRODUCAO":
        raise ConfigError("MODO_EXECUCAO invalido para este pacote. Este pacote cliente aceita somente Producao.")
    cfg["MODO_EXECUCAO"] = "Producao"
    ...
    # TRAVA CONGELADA MC4 v1.3.10:
    # O config NAO pode carregar periodo fiscal fixo. Mesmo que um arquivo antigo
    # traga periodo_base_inicio/periodo_base_fim ou data_inicial_calculo/data_final_calculo,
    # esses valores sao ignorados na inicializacao. O periodo valido e sempre o
    # periodo informado na execucao pelo BAT, pela interface web ou pelas variaveis
    # RIR70_DATA_INICIAL/RIR70_DATA_FINAL.
    cfg["data_inicial_calculo"] = ""
    cfg["data_final_calculo"] = ""
    cfg["periodo_base_inicio"] = ""
    cfg["periodo_base_fim"] = ""
    cfg["periodo_config_bloqueado"] = "Sim - periodo fixo removido do config"
    ...
    cfg.setdefault("fonte_excel", "Aptos Narrow")
...
def classify_cfop(cfop, config):
    ...
    # Usa frozensets pré-computados por ensure_config; critério subsidiário inline se config vier sem eles (testes unitários)
...
def _matriz_produto_cache(config):
    ...
    # ``frozenset``. Um critério subsidiário com ``hash(frozenset(config.items()))`` também
...
def _validar_meta_matriz_para_calculo(meta, config):
    ...
    # correta aplicação das regras de CFOP e de critério subsidiário de inventário.  A
...
def _infer_tipo_item(ncm, descricao, config, codigo=None, unidade=None, cnpj=None, return_source=False):
    ...
    # 2) Regra operacional explícita do comando v5/v6 como critério subsidiário auditável.
...
# --- Hardening fiscal-arquitetural v1.3.12: FONTENORMA, TRAIL e hash por linha ---
RIR70_RULE_DEFAULT = {
    "ID_REGRA_FISCAL": "RIR70_ART308_II_PRODUTO_ACABADO_70_MAIOR_PRECO",
    "FONTE_NORMA": "Decreto 9.580/2018 (RIR/2018), art. 308, caput, inciso II e paragrafo 1o.",
    "FONTE_PREMISSA_OPERACIONAL": "Politica fiscal-contabil MC4: criterios subsidiarios de valoracao somente quando inexistir preco direto do proprio produto na competencia; exige rastreabilidade e aprovacao quando aplicavel.",
    "FORMULA_APLICADA": "CUSTO_ARBITRADO_70 = MAIOR_PRECO_UNITARIO * 0.70",
    "ARREDONDAMENTO_APLICADO": "Decimal ROUND_HALF_UP; unitarios ate 6 casas; quantidades ate 6 casas; totais monetarios ate 2 casas.",
    "FATOR_ARBITRAMENTO": "0.70",
}

def _linha_hash_payload(values):
    return "|".join(clean_str(v) for v in values)

def _hash_linha_calculo(r, rule_id="RIR70_ART308_II_PRODUTO_ACABADO_70_MAIOR_PRECO"):
    payload = _linha_hash_payload([
        rule_id, SCRIPT_VER, r.get("Chave NF-e"), r.get("_item_nf"), r.get("CNPJ Emitente"),
        r.get("Competência"), r.get("Código Item"), r.get("NCM"), r.get("CFOP"),
        r.get("_valor_total") or r.get("Valor Comercial"), r.get("QTD_CALCULO") or r.get("QTDE TRIB"),
        r.get("VALOR_UNITARIO_CALCULO") or r.get("Vlr Unitário Base RIR70"), r.get("_motivo")
    ])
    return hashlib.sha256(payload.encode("utf-8", errors="replace")).hexdigest()

def _trail_linha_calculo(r):
    valor = r.get("_valor_total") or r.get("Valor Comercial")
    qtd = r.get("QTD_CALCULO") or r.get("QTDE TRIB")
    unit = r.get("VALOR_UNITARIO_CALCULO") or r.get("Vlr Unitário Base RIR70")
    return f"VALOR_UNITARIO_CALCULO={valor}/{qtd}={unit}; participa={r.get('ENTRA_NO_CALCULO')}; motivo={r.get('MOTIVO') or r.get('_motivo')}"
...
def build_record(row, mapping, config, manual_factors, ipi_column_present, row_origin=""):
    ...
    r["FATOR_QTD_EMBALAGEM_UN"] = matriz_meta.get("FATOR_UNIDADE") if matriz_meta else ""
    r["VOLUME_UNITARIO_ML"] = matriz_meta.get("VOLUME_ML") if matriz_meta else ""
    try:
        _fat = _matriz_decimal(matriz_meta.get("FATOR_UNIDADE")) if matriz_meta else None
        _vol = _matriz_decimal(matriz_meta.get("VOLUME_ML")) if matriz_meta else None
        r["VOLUME_TOTAL_EMBALAGEM_ML"] = q4(_fat * _vol) if _fat is not None and _vol is not None else ""
    except Exception:
        r["VOLUME_TOTAL_EMBALAGEM_ML"] = ""
    ...
    r["_monthly_key"] = (cnpj_key, item_key, normalize_ncm(r.get("NCM")), r.get("Competência"))
    r["_group_key"] = (cnpj_key, item_key, normalize_ncm(r.get("NCM")))
    r.update(RIR70_RULE_DEFAULT)
    r["TRAIL_CALCULO"] = _trail_linha_calculo(r)
    r["HASH_LINHA_CALCULO"] = _hash_linha_calculo(r, r.get("ID_REGRA_FISCAL"))
    return r
...
def duplicate_fingerprint(r):
    ...
    if re.fullmatch(r"\d{44}", chave):
        if item:
            return f"{chave}|{item}"
        # Criterio Subsidiario sem nItem: chave composta ampliada para reduzir colisão entre
...
MOV_COLS_ANALISTA = [
    ...
    "UM_ORIGINAL","FATOR_CAIXA","FATOR_QTD_EMBALAGEM_UN","VOLUME_UNITARIO_ML","VOLUME_TOTAL_EMBALAGEM_ML","QTD_CALCULO","VALOR_UNITARIO","SITUACAO_QTD",
    "CALCULA","MOTIVO","ALERTA","ADVERTENCIA",
    "ID_REGRA_FISCAL","FONTE_NORMA","FONTE_PREMISSA_OPERACIONAL","FORMULA_APLICADA","TRAIL_CALCULO","ARREDONDAMENTO_APLICADO","HASH_LINHA_CALCULO",
...
ARB_COLS_ANALISTA = [
    ...
    "Part_Arbitramento","VINCULO_GRUPO","CATEGORIA_ITEM","CRITERIO_CUSTEIO_RIR70","NCM","CEST","FATOR_QTD_EMBALAGEM_UN","VOLUME_UNITARIO_ML","VOLUME_TOTAL_EMBALAGEM_ML","MAIOR_PRECO_UNITARIO",
    ...
    "ID_REGRA_FISCAL","FONTE_NORMA","FONTE_PREMISSA_OPERACIONAL","FORMULA_APLICADA","VALORES_INTERMEDIARIOS","ARREDONDAMENTO_APLICADO","FATOR_ARBITRAMENTO","HASH_LINHA_CALCULO",
...
INV_COLS_ANALISTA = [
    # Aba enxuta: estoque e valoração. CMV e quantidade vendida ficam somente em CUSTO_CMV.
    "ID_INVENTARIO", "EMPRESA", "MES", "ALMOX", "CODIGO_PRODUTO",
    "DESCRICAO_TECNICA", "NCM", "CEST", "TIPO_PRODUTO", "CRITERIO_CUSTEIO_RIR70",
    "UNIDADE", "FATOR_QTD_EMBALAGEM_UN", "VOLUME_UNITARIO_ML", "VOLUME_TOTAL_EMBALAGEM_ML", "SALDO_ESTOQUE", "CUSTO_ARBITRADO_70", "VALOR_ESTOQUE_ARBITRADO",
    "ORIGEM_DO_PRECO", "EMPRESA_ORIGEM_PRECO", "PRODUTO_ORIGEM_PRECO", "MES_ORIGEM_PRECO",
    "DOCUMENTO_ORIGEM", "SITUACAO_PRECO", "ALERTA", "ADVERTENCIA",
    "INTERVENCAO_ANALISTA", "ACAO_ANALISTA"
]

CUSTO_CMV_COLS_ANALISTA = [
    # Aba enxuta: somente custo, CMV, estoque e trilha fiscal-contábil necessária.
    "ID_CUSTO_CMV", "EMPRESA", "MES", "CODIGO_PRODUTO", "DESCRICAO_TECNICA",
    "NCM", "CEST", "TIPO_PRODUTO", "CRITERIO_CUSTEIO_RIR70", "UNIDADE",
    "FATOR_QTD_EMBALAGEM_UN", "VOLUME_UNITARIO_ML", "VOLUME_TOTAL_EMBALAGEM_ML",
    "QUANTIDADE_VENDIDA", "CUSTO_ARBITRADO_70", "CMV", "SALDO_ESTOQUE", "VALOR_ESTOQUE_ARBITRADO",
    "ORIGEM_DO_PRECO", "DOCUMENTO_ORIGEM", "SITUACAO_PRECO", "ALERTA", "ADVERTENCIA",
    "ID_REGRA_FISCAL", "FONTE_NORMA", "FONTE_PREMISSA_OPERACIONAL",
    "FORMULA_CMV", "FORMULA_ESTOQUE", "VALORES_INTERMEDIARIOS", "ARREDONDAMENTO_APLICADO", "HASH_LINHA_CALCULO",
    "ACAO_ANALISTA"
]
# Colunas tecnicas opcionais: somente aparecem quando exibir_colunas_diagnostico = Sim.
MOV_COLS_DIAGNOSTICO = ["TIPO_ITEM","FONTE_TIPO_ITEM","STATUS_CADASTRO_MATRIZ","CALCULA_MATRIZ","FATOR_UNIDADE_MATRIZ","FATOR_QTD_EMBALAGEM_UN","VOLUME_UNITARIO_ML","VOLUME_TOTAL_EMBALAGEM_ML","NCM_MATRIZ","FUNDAMENTO_CRITERIO_CUSTEIO"]
ARB_COLS_DIAGNOSTICO = ["TIPO_ITEM","FONTE_TIPO_ITEM","STATUS_CADASTRO_MATRIZ","CALCULA_MATRIZ","FATOR_UNIDADE_MATRIZ","FATOR_QTD_EMBALAGEM_UN","VOLUME_UNITARIO_ML","VOLUME_TOTAL_EMBALAGEM_ML","NCM_MATRIZ","FUNDAMENTO_CRITERIO_CUSTEIO"]
INV_COLS_DIAGNOSTICO = ["TIPO_ITEM","FONTE_TIPO_ITEM","STATUS_CADASTRO_MATRIZ","CALCULA_MATRIZ","FATOR_UNIDADE_MATRIZ","FATOR_QTD_EMBALAGEM_UN","VOLUME_UNITARIO_ML","VOLUME_TOTAL_EMBALAGEM_ML","NCM_MATRIZ","FUNDAMENTO_CRITERIO_CUSTEIO"]
CUSTO_CMV_COLS_DIAGNOSTICO = ["TIPO_ITEM","FONTE_TIPO_ITEM","STATUS_CADASTRO_MATRIZ","CALCULA_MATRIZ","FATOR_UNIDADE_MATRIZ","FATOR_QTD_EMBALAGEM_UN","VOLUME_UNITARIO_ML","VOLUME_TOTAL_EMBALAGEM_ML","NCM_MATRIZ","FUNDAMENTO_CRITERIO_CUSTEIO"]
...
FONTES_COLS_FINAL = ["ARQUIVO","TIPO","EMPRESA","PERIODO","REGISTROS_LIDOS","REGISTROS_VALIDOS","LINHAS_LIDAS","LINHAS_USADAS","LINHAS_DESCARTADAS","MOTIVO_PRINCIPAL_DESCARTE","HASH_FONTE","VERSAO_FONTE","CODIGO_CONTROLE_ARQUIVO","SITUACAO"]
...
TIPOS_AJUSTE_VALIDOS = {
    ...
    "BLOQUEAR_ITEM", "CORRIGIR_CADASTRO_PRODUTO",
    "APROVAR_CRITERIO_SUBSIDIARIO", "REJEITAR_CRITERIO_SUBSIDIARIO",
    "APROVAR_FALLBACK", "REJEITAR_FALLBACK",  # aliases legados aceitos na leitura
    "INFORMAR_PRECO_REFERENCIA", "CORRIGIR_SALDO_INVENTARIO",
    "REVISAR_CLASSIFICACAO_OPERACAO"
}

def _ajuste_altera_calculo(tipo_ajuste):
    t = norm_text(tipo_ajuste)
    if t in {"APROVARALERTAPRECO", "APROVARCRITERIOSUBSIDIARIO", "APROVARFALLBACK", "REVISARCLASSIFICACAOOPERACAO"}:
        return "Nao"
    if t in {"INFORMARPRECOREFERENCIA", "CORRIGIRQTCALCULO", "CORRIGIRFATORUNIDADE", "CORRIGIRSALDOINVENTARIO", "BLOQUEARITEM", "REJEITARCRITERIOSUBSIDIARIO", "REJEITARFALLBACK"}:
        return "Sim"
...
def _ajustes_file_path(config):
    """
    Retorna o caminho completo do arquivo de ajustes em que o analista
    deverá registrar suas intervenções. A partir da versão 1.3.9, o nome
    do arquivo passa a ser gerado dinamicamente com base no período de
    análise e no instante de execução, seguindo a recomendação de
    governança descrita no relatório técnico. O formato adotado é:

        Ajustes_RIR70_de_<data_inicio>_ate_<data_fim>_Processado_<ddmmaaaa>_<hhmm>.xlsx

    onde:
      • <data_inicio> e <data_fim> são extraídos do período base do
        config (periodo_base_inicio e periodo_base_fim), removendo
        caracteres não numéricos (ex.: "01/01/2025" → "01012025");
      • <ddmmaaaa>_<hhmm> corresponde à data e hora de execução
        (RUN_TS).

    Caso a chave `arquivo_ajustes_analista` exista no config e traga
    algum valor diferente de vazio ou do nome padrão antigo
    ("AJUSTES_ANALISTA_RIR70.xlsx"), este valor será utilizado como
    override e nenhuma transformação ocorrerá. Isso permite que o
    usuário forneça um nome específico quando necessário.

    Parâmetros:
        config (dict): dicionário de configuração carregado a partir de
            `config_arbitramento_rir70.json`.

    Retorna:
        pathlib.Path: o caminho absoluto do arquivo de ajustes.
    """
    # Se o usuário forneceu explicitamente um nome de arquivo
    # diferente do antigo padrão, honramos essa escolha.
    nome_cfg = clean_str(config.get("arquivo_ajustes_analista"))
    if nome_cfg and nome_cfg.upper() not in {"AJUSTES_ANALISTA_RIR70.XLSX", "AJUSTES_RIR70.XLSX"}:
        return _ajustes_folder(config) / nome_cfg

    # Constrói o nome dinâmico utilizando o período de base
    def _fmt_data(dt_str: str) -> str:
        # Remove separadores como barra ou hífen e mantém somente dígitos.
        t = re.sub(r"[^0-9]", "", clean_str(dt_str))
        # Se a data não contiver 8 dígitos, retornamos a string crua para
        # evitar mascaramento incorreto. O analista será capaz de
        # identificar qualquer eventual inconsistência.
        return t if len(t) == 8 else t

    inicio = _fmt_data(config.get("periodo_base_inicio"))
    fim    = _fmt_data(config.get("periodo_base_fim"))
    exec_dt = RUN_TS.strftime("%d%m%Y_%H%M")
    dynamic_name = f"Ajustes_RIR70_de_{inicio}_ate_{fim}_Processado_{exec_dt}.xlsx"
    return _ajustes_folder(config) / dynamic_name
...
def _tipo_ajuste_sugerido(tipo_pendencia):
    ...
    if "MESANTERIOR" in t:
        return "APROVAR_CRITERIO_SUBSIDIARIO"
...
def _acao_analista(tipo_pendencia):
    ...
    if "SEMPRECO" in t or "PRECOLOCALIZADO" in t:
        return "Informar preço de referência com fonte documental ou corrigir base de vendas/critério subsidiário. O motor relerá a informação na próxima execução."
    ...
    if "MESANTERIOR" in t:
        return "Preço de mês anterior utilizado como critério subsidiário de valoração. Validar se o valor é comparável à competência atual. Se aceitável, aprovar em AJUSTES_ANALISTA_RIR70.xlsx com TIPO_AJUSTE = APROVAR_CRITERIO_SUBSIDIARIO."
    if "OUTRAEMPRESA" in t or "SIMILAR" in t:
        return "Validar comparabilidade do preço/critério subsidiário. Se correto, aprovar no arquivo de ajustes; se incorreto, rejeitar critério subsidiário e informar preço referência."
    if "NCMDIVERGENTE" in t:
...
def _risco_se_nao_corrigir(tipo_pendencia):
    t = norm_text(tipo_pendencia)
    if "REJEITARCRITERIOSUBSIDIARIO" in t or "SEMPRECO" in t or "PRECOLOCALIZADO" in t:
...
def _apply_operator_adjustments_to_inventario(inv_rows, config):
    ...
    for r in inv_rows:
        ...
        if idr in ajustes:
            aj = ajustes[idr]
            tipo = clean_str(aj.get("TIPO_AJUSTE")).upper()
            if tipo in {"APROVAR_CRITERIO_SUBSIDIARIO", "APROVAR_FALLBACK", "INFORMAR_PRECO_REFERENCIA"}:
                aprovado_subs = tipo in {"APROVAR_CRITERIO_SUBSIDIARIO", "APROVAR_FALLBACK"}
                r["SITUACAO_PRECO"] = "APROVADA" if aprovado_subs else "REVISAR"
                r["ALERTA"] = "OK" if aprovado_subs else "REVISAR"
                r["INTERVENCAO_ANALISTA"] = "NAO" if aprovado_subs else "SIM"
                r["ACAO_ANALISTA"] = "Critério subsidiário/preço validado pelo analista com justificativa registrada."
                r["AJUSTE_ANALISTA_APLICADO"] = "SIM"
                r["ID_AJUSTE"] = clean_str(aj.get("ID_AJUSTE")) or _make_id("AJ", idr, tipo, aj.get("VALOR_AJUSTADO"))
                r["JUSTIFICATIVA_ANALISTA"] = clean_str(aj.get("JUSTIFICATIVA"))
                continue
...
def _write_ajustes_xlsx(path, rows, config, title="AJUSTES ANALISTA RIR70"):
    ...
    font = config.get("fonte_excel", "Aptos Narrow")
    ...
    with pd.ExcelWriter(path, engine="xlsxwriter", datetime_format="dd/mm/yyyy", date_format="dd/mm/yyyy") as writer:
        ...
        if rows:
            ...
            ws.data_validation(1, tipo_col, max(1, len(rows)), tipo_col, {
                "validate": "list",
                "source": sorted(x for x in TIPOS_AJUSTE_VALIDOS if "FALLBACK" not in x),
                "input_title": "Tipo de Ajuste",
                "input_message": "Aprovar alerta/critério subsidiário não muda cálculo. Informar preço exige valor e evidência. Rejeitar critério subsidiário pode bloquear o item.",
        ...
        # Adiciona comentário na célula A1 da planilha de ajustes para orientar o analista sobre qual
        # arquivo usar. A partir da versão 1.3.9 o nome do arquivo de ajustes é gerado
        # automaticamente com base no período e na data de processamento. Portanto, o analista
        # deve salvar e manter este arquivo na pasta ``input/ajustes`` respeitando o nome
        # sugerido pelo motor (por exemplo: ``Ajustes_RIR70_de_01012025_ate_31122025_Processado_20052026_1045.xlsx``).
        # O template gerado no diretório de saída (output) é apenas uma cópia de auditoria e
        # **não** é lido pelo motor em execuções subsequentes.
        ws.write_comment(0, 0, "Arquivo operacional: salve e mantenha este arquivo na pasta input/ajustes com o mesmo nome gerado automaticamente pelo motor. O template no output é apenas cópia de auditoria e não é lido pelo motor.")
...
def build_movimentacao_rows(records):
    ...
    for r in records:
        ...
        rows.append({
            ...
            "ADVERTENCIA": adv,
            "ID_REGRA_FISCAL": r.get("ID_REGRA_FISCAL"),
            "FONTE_NORMA": r.get("FONTE_NORMA"),
            "FONTE_PREMISSA_OPERACIONAL": r.get("FONTE_PREMISSA_OPERACIONAL"),
            "FORMULA_APLICADA": r.get("FORMULA_APLICADA"),
            "TRAIL_CALCULO": r.get("TRAIL_CALCULO"),
            "ARREDONDAMENTO_APLICADO": r.get("ARREDONDAMENTO_APLICADO"),
            "HASH_LINHA_CALCULO": r.get("HASH_LINHA_CALCULO"),
            "INTERVENCAO_ANALISTA": interv,
            "ACAO_ANALISTA": acao,
        })
    return rows

def _hash_linha_generica(prefix, values):
    payload = _linha_hash_payload([prefix, SCRIPT_VER, *values])
    return hashlib.sha256(payload.encode("utf-8", errors="replace")).hexdigest()

def _valores_intermediarios_arbitramento(maxv, avg, soma_qtd, soma_valor, docs, diff):
    return (
        f"MAIOR_PRECO_UNITARIO={maxv}; PRECO_MEDIO={avg}; "
        f"QTD_TOTAL_VENDIDA={soma_qtd}; SOMA_VALOR={soma_valor}; "
        f"DOCUMENTO_MAIOR_PRECO={'; '.join(docs)}; VARIACAO_PRECO={diff}"
    )

def _classificacao_criterio_subsidiario(origem):
    o = clean_str(origem).upper()
    if o == "PRECO_DO_PROPRIO_PRODUTO_NO_MES":
        return "CRITERIO_DIRETO_MES_CORRENTE", "BAIXO", "NAO"
    if o == "PRECO_DO_PROPRIO_PRODUTO_EM_MES_ANTERIOR":
        return "CRITERIO_SUBSIDIARIO_MES_ANTERIOR", "MEDIO", "SIM"
    if o == "PRECO_USADO_DE_OUTRA_EMPRESA":
        return "CRITERIO_SUBSIDIARIO_EMPRESA_GRUPO", "ALTO", "SIM"
    if o == "PRECO_USADO_DE_PRODUTO_SIMILAR":
        return "CRITERIO_SUBSIDIARIO_PRODUTO_SIMILAR", "ALTO", "SIM"
    if o == "PRECO_REFERENCIA_MANUAL":
        return "CRITERIO_SUBSIDIARIO_PRECO_REFERENCIA_MANUAL", "CRITICO", "SIM"
    if o == "SEM_PRECO_LOCALIZADO":
        return "SEM_BASE_DE_VALORACAO", "CRITICO", "SIM"
    return "CRITERIO_OPERACIONAL_NAO_CLASSIFICADO", "REVISAR", "SIM"

def _build_custo_cmv_rows(inv_rows, config=None):
    rows = []
    for r in inv_rows:
        id_inv = r.get("ID_INVENTARIO") or _id_inventario(r)
        id_cmv = _make_id("CMV", id_inv, r.get("EMPRESA"), r.get("MES"), r.get("CODIGO_PRODUTO"), r.get("ALMOX"))
        row = {c: r.get(c, "") for c in CUSTO_CMV_COLS_ANALISTA}
        row.update({
            "ID_CUSTO_CMV": id_cmv,
            "ID_INVENTARIO": id_inv,
            "ID_REGRA_FISCAL": r.get("ID_REGRA_FISCAL") or RIR70_RULE_DEFAULT.get("ID_REGRA_FISCAL"),
            "FONTE_NORMA": r.get("FONTE_NORMA") or RIR70_RULE_DEFAULT.get("FONTE_NORMA"),
            "FONTE_PREMISSA_OPERACIONAL": r.get("FONTE_PREMISSA_OPERACIONAL") or RIR70_RULE_DEFAULT.get("FONTE_PREMISSA_OPERACIONAL"),
            "FORMULA_CMV": r.get("FORMULA_CMV"),
            "FORMULA_ESTOQUE": r.get("FORMULA_ESTOQUE"),
            "VALORES_INTERMEDIARIOS": r.get("VALORES_INTERMEDIARIOS_CMV") or r.get("VALORES_INTERMEDIARIOS_ESTOQUE"),
            "ARREDONDAMENTO_APLICADO": r.get("ARREDONDAMENTO_APLICADO") or RIR70_RULE_DEFAULT.get("ARREDONDAMENTO_APLICADO"),
            "HASH_LINHA_CALCULO": r.get("HASH_LINHA_CALCULO"),
        })
        rows.append(row)
    return rows
...
def build_arbitramento_rows(records, config=None):
    ...
    for g in grouped.values():
        ...
        rows.append({
            ...
            "FATOR_QTD_EMBALAGEM_UN": first.get("FATOR_QTD_EMBALAGEM_UN"),
            "VOLUME_UNITARIO_ML": first.get("VOLUME_UNITARIO_ML"),
            "VOLUME_TOTAL_EMBALAGEM_ML": first.get("VOLUME_TOTAL_EMBALAGEM_ML"),
            "NCM": first.get("NCM"),
            ...
            "ADVERTENCIA": adv,
            "ID_REGRA_FISCAL": RIR70_RULE_DEFAULT.get("ID_REGRA_FISCAL"),
            "FONTE_NORMA": RIR70_RULE_DEFAULT.get("FONTE_NORMA"),
            "FONTE_PREMISSA_OPERACIONAL": "Regra fiscal direta: maior preço unitário do próprio produto no mês, apurado em vendas válidas.",
            "FORMULA_APLICADA": RIR70_RULE_DEFAULT.get("FORMULA_APLICADA"),
            "VALORES_INTERMEDIARIOS": _valores_intermediarios_arbitramento(maxv, avg, soma_qtd, soma_valor, docs, diff),
            "ARREDONDAMENTO_APLICADO": RIR70_RULE_DEFAULT.get("ARREDONDAMENTO_APLICADO"),
            "FATOR_ARBITRAMENTO": RIR70_RULE_DEFAULT.get("FATOR_ARBITRAMENTO"),
            "HASH_LINHA_CALCULO": _hash_linha_generica("ARB", [first.get("CNPJ Emitente"), first.get("Competência"), first.get("Código Item"), first.get("NCM"), maxv, base, avg, soma_qtd, soma_valor, "; ".join(docs)]),
...
def build_inventario_rows(arb_rows, inventory_map, config):
    ...
    for key, inv in inventory_map.items():
        ...
        # ── Critério subsidiário final: preço de referência manual (precos_referencia.xlsx) ────
        # Acionado somente quando todos os demais critérios subsidiários falharam.
        ...
        classificacao_subs, nivel_risco_subs, exige_aprov_subs = _classificacao_criterio_subsidiario(origem)
        ...
        if origem == "PRECO_REFERENCIA_MANUAL" and ref_entry:
            ...
            if normalize_cnpj(emp_origem) == "03408722000178":
                justificativa_fallback = (
                    f"PREÇO OBTIDO POR CRITÉRIO SUBSIDIÁRIO DE VALORAÇÃO DA EMPRESA {emp_origem} (FÁBRICA DO GRUPO). "
                ...
                justificativa_fallback = (
                    ...
                    "Não constitui preço de transferência por si só; é critério subsidiário de valoração por produto idêntico, sujeito a validação de comparabilidade."
                )
        # Quantidade vendida: sempre do próprio CNPJ/produto/mês — nunca do critério subsidiário de preço.
        # O critério subsidiário de valoração (mês anterior, outra empresa, preço manual) serve apenas para
        ...
        row = {
            ...
            "FATOR_QTD_EMBALAGEM_UN": matriz_meta_dest.get("FATOR_UNIDADE") if matriz_meta_dest else "",
            "VOLUME_UNITARIO_ML": matriz_meta_dest.get("VOLUME_ML") if matriz_meta_dest else "",
            "VOLUME_TOTAL_EMBALAGEM_ML": (q4(_matriz_decimal(matriz_meta_dest.get("FATOR_UNIDADE")) * _matriz_decimal(matriz_meta_dest.get("VOLUME_ML"))) if matriz_meta_dest and _matriz_decimal(matriz_meta_dest.get("FATOR_UNIDADE")) is not None and _matriz_decimal(matriz_meta_dest.get("VOLUME_ML")) is not None else ""),
            "UNIDADE": unid,
            "CUSTO_ARBITRADO_70": base,
            ...
            "ADVERTENCIA": adv,
            "JUSTIFICATIVA_CRITERIO_SUBSIDIARIO": justificativa_fallback,
            "JUSTIFICATIVA_PRECO_REF": justificativa_preco_ref,
            "ID_REGRA_FISCAL": RIR70_RULE_DEFAULT.get("ID_REGRA_FISCAL"),
            "FONTE_NORMA": RIR70_RULE_DEFAULT.get("FONTE_NORMA"),
            "FONTE_PREMISSA_OPERACIONAL": f"{classificacao_subs}; nivel_risco={nivel_risco_subs}; exige_aprovacao_analista={exige_aprov_subs}",
            "FORMULA_CMV": "CMV = QUANTIDADE_VENDIDA * CUSTO_ARBITRADO_70",
            "FORMULA_ESTOQUE": "VALOR_ESTOQUE_ARBITRADO = SALDO_ESTOQUE * CUSTO_ARBITRADO_70",
            "VALORES_INTERMEDIARIOS_CMV": f"QUANTIDADE_VENDIDA={qtde_vendida}; CUSTO_ARBITRADO_70={base}; CMV={cmv_val}",
            "VALORES_INTERMEDIARIOS_ESTOQUE": f"SALDO_ESTOQUE={saldo}; CUSTO_ARBITRADO_70={base}; VALOR_ESTOQUE_ARBITRADO={valor}; ORIGEM_DO_PRECO={origem}; EMPRESA_ORIGEM={emp_origem}; PRODUTO_ORIGEM={prod_origem}; MES_ORIGEM={mes_origem}; DOCUMENTO_ORIGEM={doc_origem}",
            "ARREDONDAMENTO_APLICADO": RIR70_RULE_DEFAULT.get("ARREDONDAMENTO_APLICADO"),
            "HASH_LINHA_CALCULO": _hash_linha_generica("INV", [emp, mes, almox, cod_orig, ncm_eff, saldo, base, valor, qtde_vendida, cmv_val, origem, emp_origem, prod_origem, mes_origem, doc_origem]),
        }
...
def _build_pendencias(records, arb_rows, inv_rows, config):
    ...
    # Pendências de inventário/critério subsidiário.
    for r in inv_rows:
        if clean_str(r.get("AJUSTE_ANALISTA_APLICADO")) == "SIM":
            continue
        tipo_pend = ""
    ...
    # GAP-01: preço de mês anterior utilizado como critério subsidiário - exige aprovação formal.
    ...
    for r in inv_rows:
        ...
        if clean_str(r.get("ORIGEM_DO_PRECO")) == "PRECO_DO_PROPRIO_PRODUTO_EM_MES_ANTERIOR":
            ...
            p["ID_REFERENCIA"] = r.get("ID_INVENTARIO") or _id_inventario(r)
            p["TIPO_AJUSTE_SUGERIDO"] = "APROVAR_CRITERIO_SUBSIDIARIO"
...
def _build_fontes_processadas(logs, records, config):
    ...
    for arq, aba, status, desc in logs:
        if clean_str(arq) == "-":
            continue
        if clean_str(status).upper() == "INVENTARIO_CNPJ":
            # evento de metadado interno; evita duplicar a fonte na aba de lineage
            continue
        ...
        rows.append({
            ...
            "HASH_FONTE": codigo_controle, "VERSAO_FONTE": SCRIPT_VER, "CODIGO_CONTROLE_ARQUIVO": codigo_controle, "ENCODING_USADO": "N/A", "SITUACAO": situ
        })
    ...
    if info_ref.get("arquivo") and info_ref.get("situacao") not in {"NAO_LOCALIZADO", "DESABILITADO"}:
        ...
        if not any(clean_str(r.get("ARQUIVO")) == arq_ref for r in rows):
            rows.append({"ARQUIVO": arq_ref, "TIPO": "PRECO_REFERENCIA", "EMPRESA": "", "PERIODO": periodo,
                         ...
                         "MOTIVO_PRINCIPAL_DESCARTE": info_ref.get("situacao"), "HASH_FONTE": info_ref.get("codigo_controle") or RIR70_SOURCE_CONTROL_CODES.get(arq_ref, ""), "VERSAO_FONTE": SCRIPT_VER, "CODIGO_CONTROLE_ARQUIVO": info_ref.get("codigo_controle") or RIR70_SOURCE_CONTROL_CODES.get(arq_ref, ""),
                         "ENCODING_USADO": "N/A", "SITUACAO": "OK" if info_ref.get("situacao") == "OK" else "REVISAR"})
    # Fontes auxiliares criticas: matriz cadastral e mapa de similaridade.
    for aux_name, aux_tipo in [
        ("MATRIZ_CADASTRO_PRODUTO_RIR70_SIMILARIDADE.xlsx", "MATRIZ_CADASTRAL"),
        ("MAPA_SIMILARIDADE_PRODUTO.xlsx", "MAPA_SIMILARIDADE"),
    ]:
        aux_path = BASE_DIR / "input" / "auxiliares" / aux_name
        if aux_path.exists():
            rows.append({
                "ARQUIVO": aux_name, "TIPO": aux_tipo, "EMPRESA": "", "PERIODO": periodo,
                "REGISTROS_LIDOS": 0, "REGISTROS_VALIDOS": 0, "REGISTROS_DUPLICADOS": 0,
                "REGISTROS_BLOQUEADOS_CFOP": 0, "REGISTROS_SEM_CADASTRO": 0,
                "LINHAS_LIDAS": 0, "LINHAS_USADAS": 0, "LINHAS_DESCARTADAS": 0,
                "LINHAS_DUPLICADAS_DESCARTADAS": 0, "MOTIVO_PRINCIPAL_DESCARTE": "FONTE_AUXILIAR_CRITICA_USADA_NO_PROCESSAMENTO",
                "HASH_FONTE": sha256_file(aux_path), "VERSAO_FONTE": SCRIPT_VER, "CODIGO_CONTROLE_ARQUIVO": sha256_file(aux_path), "ENCODING_USADO": "N/A", "SITUACAO": "OK"
            })
        else:
            rows.append({
                "ARQUIVO": aux_name, "TIPO": aux_tipo, "EMPRESA": "", "PERIODO": periodo,
                ...
                "REGISTROS_BLOQUEADOS_CFOP": 0, "REGISTROS_SEM_CADASTRO": 0,
                "LINHAS_LIDAS": 0, "LINHAS_USADAS": 0, "LINHAS_DESCARTADAS": 0,
                "LINHAS_DUPLICADAS_DESCARTADAS": 0, "MOTIVO_PRINCIPAL_DESCARTE": "FONTE_AUXILIAR_CRITICA_NAO_LOCALIZADA",
                "HASH_FONTE": "", "VERSAO_FONTE": SCRIPT_VER, "CODIGO_CONTROLE_ARQUIVO": "", "ENCODING_USADO": "N/A", "SITUACAO": "PENDENTE"
            })
    return rows

def _humanizar_codigo_saida(v):
    """Exibe códigos operacionais sem poluir a leitura do analista.

    Mantém o significado, mas troca underscores por espaços e preserva siglas fiscais.
    """
    t = clean_str(v)
    if not t:
        return t
    t = t.replace("_", " ").strip()
    fixes = {
        "Rir70": "RIR70", "Ncm": "NCM", "Cest": "CEST", "Cfop": "CFOP",
        "Cmv": "CMV", "Icms": "ICMS", "St": "ST", "Fcp": "FCP",
        "Nfe": "NF-e", "Nf E": "NF-e", "Nao": "Nao",
    }
    words = []
    for w in t.lower().split():
        wl = w.upper()
        if wl in {"RIR70", "NCM", "CEST", "CFOP", "CMV", "ICMS", "ST", "FCP", "PA", "MR"}:
            words.append(wl)
        else:
...
def _write_final_sheet(wb, ws, columns, rows, config, tab_kind="data"):
    ...
    fmt_h = wb.add_format({"font_name":font,"font_size":size,"bold":True,"font_color":"#FFFFFF","bg_color":header_fill,"align":"center","valign":"vcenter","text_wrap":False})
    fmt_txt = wb.add_format({"font_name":font,"font_size":size,"valign":"top","text_wrap":False})
    fmt_alt = wb.add_format({"font_name":font,"font_size":size,"valign":"top","bg_color":"#F7F9FC","text_wrap":False})
    fmt_ctr = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","align":"center"})
    fmt_ctr_alt = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","align":"center","bg_color":"#F7F9FC"})
    fmt_date = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","align":"center","num_format":"dd/mm/yyyy"})
    fmt_date_alt = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","align":"center","bg_color":"#F7F9FC","num_format":"dd/mm/yyyy"})
    fmt_num2 = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","num_format":"#,##0.00"})
    fmt_num2_alt = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","bg_color":"#F7F9FC","num_format":"#,##0.00"})
    fmt_num4     = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","num_format":"#,##0.0000"})
    fmt_num4_alt = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","bg_color":"#F7F9FC","num_format":"#,##0.0000"})
    fmt_num6     = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","num_format":"#,##0.000000"})
    fmt_num6_alt = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","bg_color":"#F7F9FC","num_format":"#,##0.000000"})
    fmt_pct = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","align":"center","num_format":"0%"})
    fmt_pct_alt = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","align":"center","bg_color":"#F7F9FC","num_format":"0%"})
    fmt_pct_alert = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#FFF2CC","font_color":"#7F6000","valign":"vcenter","align":"center","num_format":"0%"})
    fmt_capa_status_ok   = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#E2F0D9","font_color":"#375623","valign":"vcenter","align":"center"})
    fmt_capa_status_warn = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#FFF2CC","font_color":"#7F6000","valign":"vcenter","align":"center"})
    fmt_capa_status_crit = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#FCE4D6","font_color":"#9C0006","valign":"vcenter","align":"center"})
    fmt_capa_sub_header  = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#1A3A5C","font_color":"#D9EAF7","valign":"vcenter","align":"left","text_wrap":False})
    # Formato para células VALOR da CAPA com texto longo: quebra automática + alinhamento justificado.
    fmt_capa_long_val    = wb.add_format({"font_name":font,"font_size":size,"valign":"top","align":"justify","text_wrap":False})
    # Formato numérico inteiro para contadores na CAPA (separador de milhar, sem decimais)
    fmt_capa_int      = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","align":"right","num_format":"#,##0"})
    fmt_capa_int_alt  = wb.add_format({"font_name":font,"font_size":size,"bg_color":"#F7F9FC","valign":"vcenter","align":"right","num_format":"#,##0"})
    # Contadores de alerta (> 0): destaque âmbar ou vermelho dependendo da coluna
    fmt_capa_int_warn = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#FFF2CC","font_color":"#7F6000","valign":"vcenter","align":"right","num_format":"#,##0"})
    fmt_capa_int_crit = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#FCE4D6","font_color":"#9C0006","valign":"vcenter","align":"right","num_format":"#,##0"})
    fmt_alert = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#FFF2CC","font_color":"#7F6000","valign":"vcenter","align":"left"})
    fmt_icon = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#F2F2F2","font_color":"#404040","valign":"vcenter","align":"center"})
    fmt_icon_crit = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#FCE4D6","font_color":"#9C0006","valign":"vcenter","align":"center"})
    fmt_icon_rev = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#FFF2CC","font_color":"#C65911","valign":"vcenter","align":"center"})
    fmt_icon_warn = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#FFF2CC","font_color":"#7F6000","valign":"vcenter","align":"center"})
    fmt_icon_ok = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#E2F0D9","font_color":"#375623","valign":"vcenter","align":"center"})
    fmt_ok = wb.add_format({"font_name":font,"font_size":size,"bold":True,"bg_color":"#E2F0D9","font_color":"#375623","valign":"vcenter","align":"center"})
    fmt_gold6     = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","align":"right","num_format":"R$ #,##0.000000","bg_color":"#FFFBEB","font_color":"#92600A","bold":True})
    fmt_gold6_alt = wb.add_format({"font_name":font,"font_size":size,"valign":"vcenter","align":"right","num_format":"R$ #,##0.000000","bg_color":"#FEF9E7","font_color":"#92600A","bold":True})
    fmt_link = wb.add_format({"font_name":font,"font_size":size,"font_color":"#0563C1","underline":1,"valign":"vcenter","align":"center"})
    # Link com alinhamento à esquerda - usado para LINK_ABRIR_AJUSTE e links na capa.
    fmt_link_left = wb.add_format({"font_name":font,"font_size":size,"font_color":"#0563C1","underline":1,"valign":"vcenter","align":"left"})
    ...
    for ri, row in enumerate(rows, start=1):
        ...
        for ci, col in enumerate(columns):
            ...
            if isinstance(v, str) and col in {
                "CRITERIO_CUSTEIO_RIR70", "INTERVENCAO_ANALISTA", "SITUACAO_PRECO",
                "MOTIVO", "TIPO_PENDENCIA", "TIPO_AJUSTE_SUGERIDO", "CAMPO_AJUSTE",
                "MOTIVO_PRINCIPAL_DESCARTE", "ORIGEM_DO_PRECO", "ALERTA", "ADVERTENCIA",
                "SITUACAO_QTD", "CALCULA", "Part_Arbitramento", "VINCULO_GRUPO", "CATEGORIA_ITEM",
                "AJUSTE_ANALISTA_APLICADO", "ALTERA_CALCULO", "SITUACAO"
            }:
                if v:
                    v = _humanizar_codigo_saida(v)
            ...
            if (col.startswith("LINK_") or (tab_kind == "capa" and col == "VALOR" and norm_text(row.get("INDICADOR")).startswith("LINK"))) and clean_str(display):
                ...
                text = _humanizar_codigo_saida(display)
                ws.write(ri, ci, text, fmt_alert)
    ...
    for ci, col in enumerate(columns):
        ...
        if tab_kind == "capa" and col == "VALOR":
            w = max(w, 85)
        if col in {"FONTE_NORMA", "FORMULA_APLICADA", "TRAIL_CALCULO", "VALORES_INTERMEDIARIOS", "ARREDONDAMENTO_APLICADO", "HASH_LINHA_CALCULO"}:
            w = max(w, 42)
...
def _expected_headers_map(config):
    return {
        ...
        "CUSTO_CMV": _cols_operacionais(CUSTO_CMV_COLS_ANALISTA, CUSTO_CMV_COLS_DIAGNOSTICO, config),
...
def write_workbook(records, logs, config):
    ...
    excel_write_path = final_output_path
    raw_output_path = None
    if _is_sim(config.get("aplicar_layout_premium_output", "Sim")) and (
        _is_sim(config.get("layout_usar_arquivo_raw_temporario", "Nao")) or clean_str(config.get("layout_nivel")).lower() == "auditoria"
    ):
        raw_dir = OUTPUT_DIR / "_tmp"
        raw_dir.mkdir(parents=True, exist_ok=True)
        raw_output_path = raw_dir / f"{final_output_path.stem}_RAW.xlsx"
        seq = 1
        while raw_output_path.exists():
            raw_output_path = raw_dir / f"{final_output_path.stem}_RAW_{seq:02d}.xlsx"
            seq += 1
        excel_write_path = raw_output_path
    ...
    custo_cmv_rows = _build_custo_cmv_rows(inv_rows, config)
    ...
    custo_cmv_rows = _normalizar_linhas_para_colunas(custo_cmv_rows, expected_headers["CUSTO_CMV"])
    ...
    with pd.ExcelWriter(excel_write_path, engine="xlsxwriter", datetime_format="dd/mm/yyyy", date_format="dd/mm/yyyy") as writer:
        wb = writer.book
        wb.set_properties({
            "title": "Motor Fiscal RIR70 by MC4",
            "subject": "Arbitramento de Custo - Licenca de Uso MC4",
            "author": MOTOR_OWNER,
            "company": MOTOR_OWNER,
            "comments": "Ferramenta proprietaria MC4 licenciada para uso interno do cliente autorizado."
        })
        sheets = [
            ...
            ("CUSTO_CMV", expected_headers["CUSTO_CMV"], custo_cmv_rows, "cmv"),
    ...
    # Valida o Excel técnico antes do layout. Depois do layout, os cabeçalhos visíveis podem ser amigáveis.
    validate_generated_xlsx_package(excel_write_path, expected_headers=expected_headers, config=config)

    if excel_write_path != final_output_path:
        aplicar_layout_premium_mc4_se_configurado(excel_write_path, final_output_path, config, perfil="output_fiscal")
        manter_raw = _is_sim(config.get("layout_manter_raw_temporario", "Nao")) or clean_str(config.get("layout_nivel")).lower() == "auditoria"
        if raw_output_path and not manter_raw:
            try:
                raw_output_path.unlink()
            except Exception:
                pass
    else:
        aplicar_layout_premium_mc4_se_configurado(final_output_path, final_output_path, config, perfil="output_fiscal")

    if not final_output_path.exists():
        raise ConfigError(f"Arquivo final não foi gerado: {final_output_path}")
    print(f"OK Excel final gerado, formatado e validado estruturalmente: {final_output_path}")
...
def validate_generated_xlsx_package(path, expected_headers=None, config=None):
    required = ["CAPA", "01_MOVIMENTO_VALIDADO", "02_ARBITRAMENTO", "03_INVENTARIO_VALORIZADO", "CUSTO_CMV", "04_PENDENCIAS", "05_FONTES_PROCESSADAS"]
...
def write_metadata(records, config):
    meta = {
        ...
        "abas_obrigatorias": ["CAPA", "01_MOVIMENTO_VALIDADO", "02_ARBITRAMENTO", "03_INVENTARIO_VALORIZADO", "CUSTO_CMV", "04_PENDENCIAS", "05_FONTES_PROCESSADAS"],
        ...
        "mc4_proprietario": MOTOR_OWNER,
        "mc4_proprietario_cnpj": MOTOR_OWNER_CNPJ,
        "mc4_produto": MOTOR_BRAND,
        "mc4_licenca": config.get("licenca_mc4"),
        "mc4_integridade": config.get("_integridade_mc4"),
        "mc4_cnpjs_nao_licenciados": config.get("_licenca_mc4_cnpjs_nao_autorizados", []),
        "layout_premium_mc4_output": config.get("_layout_output_fiscal"),
        "layout_premium_mc4_pre_validacao": config.get("_layout_pre_validacao"),
        "layout_premium_mc4_ajustes": config.get("_layout_ajustes_analista"),
        "layout_nivel": config.get("layout_nivel"),
        "layout_performance": {
            "raw_temporario": config.get("layout_usar_arquivo_raw_temporario"),
            "manter_raw": config.get("layout_manter_raw_temporario"),
            "modo_rapido_acima_linhas": config.get("layout_modo_rapido_acima_linhas"),
            "zebra": config.get("layout_aplicar_zebra"),
            "validacao_profunda": config.get("layout_validacao_profunda"),
            "origem": config.get("_layout_nivel_origem")
        },
    }
...
def _file_date_range_from_name(file_name):
    ...
    m_inv = re.search(r"Inventario_\d{14}_((0[1-9]|1[0-2])(20\d{2}))\.xls", txt, flags=re.I)
    if m_inv:
        comp = m_inv.group(1); mes = int(comp[:2]); ano = int(comp[2:])
        try:
            return datetime(ano, mes, 1), datetime(ano, mes, calendar.monthrange(ano, mes)[1], 23, 59, 59)
        except Exception:
            return None, None
    m_mov = re.search(r"MovimentoItem_\d{14}_((0[1-9]|1[0-2])(20\d{2}))\.xls", txt, flags=re.I)
    if m_mov:
        comp = m_mov.group(1); mes = int(comp[:2]); ano = int(comp[2:])
        try:
            return datetime(ano, mes, 1), datetime(ano, mes, calendar.monthrange(ano, mes)[1], 23, 59, 59)
        except Exception:
            return None, None
...
def _file_overlaps_period(path, config):
    ...
    file_name = getattr(path, "name", path)
    a, b = _file_date_range_from_name(file_name)
    # Se conseguirmos extrair o intervalo do nome, verifica sobreposição com o período informado
    if a and b and inicio and fim:
        return not (b < inicio or a > fim)
    # Caso não seja possível extrair o intervalo, tenta filtrar por anos no nome
    if inicio and fim:
        import re
        selected_years = set(range(inicio.year, fim.year + 1))
        years_in_name = set(int(y) for y in re.findall(r"(?<!\d)(?:19|20)\d{2}(?!\d)", file_name))
        # Se não houver anos identificados, considera o arquivo válido
        if not years_in_name:
            return True
        # Caso haja anos, aceita apenas se algum estiver dentro do intervalo selecionado
        return any(y in selected_years for y in years_in_name)
    # Quando não houver período informado, inclui o arquivo por padrão
    return True
...
def resolve_inventory_cnpj(file_path, config):
    meta = parse_inventory_filename(file_path, config) if clean_str(config.get("inventario_formato_competencia")).lower() == "arquivos_mensais_por_cnpj" else None
    if meta:
        return meta["cnpj"], "cnpj_competencia_no_nome_arquivo"
...
def load_inventory_pa(config, logs):
    ...
    for file_path in discover_inventory_files(config):
        ...
        meta_inv_file = parse_inventory_filename(file_path, config) if clean_str(config.get("inventario_formato_competencia")).lower() == "arquivos_mensais_por_cnpj" else None
        comp_arquivo = meta_inv_file.get("competencia") if meta_inv_file else None
        detail_logs.append((file_path.name, "-", "INVENTARIO_CNPJ", f"CNPJ={cnpj_inv or 'NAO_IDENTIFICADO'}; competencia={comp_arquivo or 'ABA'}; {cnpj_origem}"))
        try:
        ...
        for sh in wb_ro.sheetnames:
            comp = comp_arquivo or competencia_from_sheet_name(sh)
...
def _perguntar_data_periodo(rotulo, padrao_dt, tipo_limite):
    while True:
        ...
        print("Data invalida. Informe DD/MM/AAAA ou MM/AAAA. Ex.: 01/01/2025, 31/12/2025 ou 03/2025.")
...
def solicitar_periodo_operacao(config):
    """Define obrigatoriamente o período de execução.

    Este utilitário exige que o período seja informado a cada execução, seja por
    variáveis de ambiente (``RIR70_DATA_INICIAL`` e ``RIR70_DATA_FINAL``)
    ou via entrada interativa no terminal. Se nenhuma dessas formas estiver
    disponível, o motor será interrompido com erro. Não existe mais a
    possibilidade de usar um período predefinido no arquivo de configuração.
    """
    cfg = dict(config or {})
    # Tenta obter período das variáveis de ambiente
    env_inicio = clean_str(os.environ.get("RIR70_DATA_INICIAL"))
    env_fim = clean_str(os.environ.get("RIR70_DATA_FINAL"))
    if env_inicio and env_fim:
        inicio = _parse_periodo_operacao_input(env_inicio, "inicio")
        fim = _parse_periodo_operacao_input(env_fim, "fim")
        return _aplicar_periodo_execucao(cfg, inicio, fim, "Sim - informado por variáveis de ambiente")

    # Caso não haja variáveis de ambiente, solicita interativamente
    try:
        interativo = bool(sys.stdin and sys.stdin.isatty())
    except Exception:
        interativo = False
    if interativo:
        # Define valores padrão apenas para facilitar a digitação; não usa
        # qualquer período fixo do config, pois o usuário deve sempre informar
        ano_atual = datetime.now().year
        inicio_padrao = datetime(ano_atual, 1, 1)
        fim_padrao = datetime(ano_atual, 12, 31)
    ...
    # Se não for possível obter o período nem via variáveis nem via entrada
    # interativa (por exemplo, rodando em modo não interativo), aborta
    raise ConfigError(
        "Periodo nao informado. Defina RIR70_DATA_INICIAL e RIR70_DATA_FINAL ou execute o motor em um terminal interativo para informar as datas."
    )



def _classificar_xml_pre_validacao(path, inner_name=""):
    label = f"{path.name}|{inner_name}" if inner_name else path.name
    try:
        data = path.read_bytes() if not inner_name else data
    except Exception:
        data = b""
    try:
        root = ET.fromstring(data)
    except Exception as exc:
        return {"ARQUIVO": label, "TIPO_FONTE": "XML", "STATUS_PRE_VALIDACAO": "XML_MALFORMADO", "DETALHE": str(exc), "CHAVE_NFE": "", "DATA_EMISSAO": "", "ITENS": 0, "SHA256": _sha256_bytes(data) if data else ""}
    status = _xml_status(root)
    chave = _xml_attr_chave(root)
    ide = root.find(".//{*}ide")
    data_emissao = _xml_find_text(ide, "./{*}dhEmi") or _xml_find_text(ide, "./{*}dEmi")
    itens = len(root.findall(".//{*}det"))
    status_out = "XML_AUTORIZADO" if status == "AUTORIZADA" else f"XML_{status}"
    if itens == 0:
        status_out = "XML_SEM_ITENS"
    return {"ARQUIVO": label, "TIPO_FONTE": "XML", "STATUS_PRE_VALIDACAO": status_out, "DETALHE": "", "CHAVE_NFE": chave, "DATA_EMISSAO": data_emissao, "ITENS": itens, "SHA256": _sha256_bytes(data)}

def _pre_validacao(config) -> None:
    """Pré-validação executiva v1.3.12.

    Gera 00_PRE_VALIDACAO.xlsx com CAPA, FONTES_ANALISADAS e INCONSISTENCIAS,
    incluindo XML/ZIP, hashes e classificação explícita de rejeições.
    """
    fontes = []
    inconsistencias = []
    # Validação fail-fast de produção: pasta de inventário limpa e arquivos por CNPJ/competência.
    if clean_str(config.get("inventario_formato_competencia")).lower() == "arquivos_mensais_por_cnpj":
        folder, inv_files, invalidos, duplicados = _scan_inventory_folder_producao(config)
        for nome, motivo in invalidos:
            inconsistencias.append({"ARQUIVO": nome, "TIPO_FONTE": "INVENTARIO_EXCEL", "STATUS_PRE_VALIDACAO": "BLOQUEADO_ARQUIVO_INVALIDO_NA_PASTA_INVENTARIO", "DETALHE": motivo, "CHAVE_NFE": "", "DATA_EMISSAO": "", "ITENS": "", "SHA256": ""})
        for nome, original, cnpj, comp in duplicados:
            inconsistencias.append({"ARQUIVO": nome, "TIPO_FONTE": "INVENTARIO_EXCEL", "STATUS_PRE_VALIDACAO": "BLOQUEADO_INVENTARIO_DUPLICADO", "DETALHE": f"Duplica {original} para CNPJ {cnpj} competencia {comp}", "CHAVE_NFE": "", "DATA_EMISSAO": "", "ITENS": "", "SHA256": ""})
        validos_periodo = [p for p in inv_files if _inventory_file_matches_period(p, config)]
        if not validos_periodo and _is_sim(config.get("bloquear_competencia_inventario_ausente", True)):
            inconsistencias.append({"ARQUIVO": str(folder), "TIPO_FONTE": "INVENTARIO_EXCEL", "STATUS_PRE_VALIDACAO": "BLOQUEADO_INVENTARIO_COMPETENCIA_AUSENTE", "DETALHE": "Nenhum Inventario_CNPJ_MMAAAA.xlsx localizado para o periodo selecionado", "CHAVE_NFE": "", "DATA_EMISSAO": "", "ITENS": "", "SHA256": ""})
    # XML/ZIP
    for p in discover_xml_files(config):
        try:
            sha = sha256_file(p)
        except Exception:
            sha = ""
        if p.suffix.lower() == ".zip":
            try:
                with zipfile.ZipFile(p) as zf:
                    names = [n for n in sorted(zf.namelist()) if n.lower().endswith(".xml")]
                    if not names:
                        row = {"ARQUIVO": p.name, "TIPO_FONTE": "ZIP_XML", "STATUS_PRE_VALIDACAO": "ZIP_SEM_XML", "DETALHE": "Nenhum XML interno localizado", "CHAVE_NFE": "", "DATA_EMISSAO": "", "ITENS": 0, "SHA256": sha}
                        fontes.append(row); inconsistencias.append(row)
                    for name in names:
                        data = zf.read(name)
                        tmp_status = _classificar_xml_pre_validacao.__globals__
                        try:
                            root = ET.fromstring(data)
                            status = _xml_status(root)
                            chave = _xml_attr_chave(root)
                            ide = root.find(".//{*}ide")
                            dh = _xml_find_text(ide, "./{*}dhEmi") or _xml_find_text(ide, "./{*}dEmi")
                            itens = len(root.findall(".//{*}det"))
                            status_out = "XML_AUTORIZADO" if status == "AUTORIZADA" else f"XML_{status}"
                            if itens == 0: status_out = "XML_SEM_ITENS"
                            row = {"ARQUIVO": f"{p.name}|{name}", "TIPO_FONTE": "ZIP_XML", "STATUS_PRE_VALIDACAO": status_out, "DETALHE": "", "CHAVE_NFE": chave, "DATA_EMISSAO": dh, "ITENS": itens, "SHA256": _sha256_bytes(data), "SHA256_CONTAINER": sha}
                        except Exception as exc:
                            row = {"ARQUIVO": f"{p.name}|{name}", "TIPO_FONTE": "ZIP_XML", "STATUS_PRE_VALIDACAO": "XML_MALFORMADO", "DETALHE": str(exc), "CHAVE_NFE": "", "DATA_EMISSAO": "", "ITENS": 0, "SHA256": _sha256_bytes(data), "SHA256_CONTAINER": sha}
                        fontes.append(row)
                        if row["STATUS_PRE_VALIDACAO"] != "XML_AUTORIZADO":
                            inconsistencias.append(row)
            except Exception as exc:
                row = {"ARQUIVO": p.name, "TIPO_FONTE": "ZIP_XML", "STATUS_PRE_VALIDACAO": "ZIP_INVALIDO", "DETALHE": str(exc), "CHAVE_NFE": "", "DATA_EMISSAO": "", "ITENS": 0, "SHA256": sha}
                fontes.append(row); inconsistencias.append(row)
        else:
            try:
                data = p.read_bytes()
                try:
                    ...
                    status_out = "XML_AUTORIZADO" if status == "AUTORIZADA" else f"XML_{status}"
                    if itens == 0: status_out = "XML_SEM_ITENS"
                    row = {"ARQUIVO": p.name, "TIPO_FONTE": "XML", "STATUS_PRE_VALIDACAO": status_out, "DETALHE": "", "CHAVE_NFE": chave, "DATA_EMISSAO": dh, "ITENS": itens, "SHA256": _sha256_bytes(data)}
                except Exception as exc:
                    row = {"ARQUIVO": p.name, "TIPO_FONTE": "XML", "STATUS_PRE_VALIDACAO": "XML_MALFORMADO", "DETALHE": str(exc), "CHAVE_NFE": "", "DATA_EMISSAO": "", "ITENS": 0, "SHA256": _sha256_bytes(data)}
                fontes.append(row)
                if row["STATUS_PRE_VALIDACAO"] != "XML_AUTORIZADO":
                    inconsistencias.append(row)
            except Exception as exc:
                row = {"ARQUIVO": p.name, "TIPO_FONTE": "XML", "STATUS_PRE_VALIDACAO": "ERRO_LEITURA", "DETALHE": str(exc), "CHAVE_NFE": "", "DATA_EMISSAO": "", "ITENS": 0, "SHA256": sha}
                fontes.append(row); inconsistencias.append(row)
    # Excel movimento/inventario: validação mínima preservada
    for key, tipo in [("pasta_movimentacao", "MOVIMENTO_EXCEL"), ("pasta_inventario", "INVENTARIO_EXCEL")]:
        folder = BASE_DIR / clean_str(config.get(key, ""))
        if folder.exists():
            for p in sorted(folder.glob("*.xls*")):
                if p.name.startswith("~$"): continue
                row = {"ARQUIVO": p.name, "TIPO_FONTE": tipo, "STATUS_PRE_VALIDACAO": "ARQUIVO_LOCALIZADO", "DETALHE": "Validação estrutural completa ocorre na leitura do motor", "CHAVE_NFE": "", "DATA_EMISSAO": "", "ITENS": "", "SHA256": sha256_file(p)}
                fontes.append(row)
    if fontes or inconsistencias:
        pre_path = OUTPUT_DIR / "00_PRE_VALIDACAO.xlsx"
        capa = [
            {"INDICADOR":"Produto", "VALOR":"Motor Fiscal RIR70 by MC4"},
            {"INDICADOR":"Versao", "VALOR":SCRIPT_VER},
            {"INDICADOR":"Fontes analisadas", "VALOR":len(fontes)},
            {"INDICADOR":"Inconsistencias", "VALOR":len(inconsistencias)},
            {"INDICADOR":"Criterio", "VALOR":"XML autorizado importa; XML sem protocolo/cancelado/malformado fica evidenciado e pode bloquear cálculo conforme regra fiscal."},
        ]
        with pd.ExcelWriter(pre_path, engine="xlsxwriter") as writer:
            pd.DataFrame(capa).to_excel(writer, sheet_name="CAPA", index=False)
            pd.DataFrame(fontes).to_excel(writer, sheet_name="FONTES_ANALISADAS", index=False)
            pd.DataFrame(inconsistencias).to_excel(writer, sheet_name="INCONSISTENCIAS", index=False)
            wb = writer.book
            # Saída bruta neutra: não aplicar estilo de coluna com borda.
            # O layout final é responsabilidade exclusiva do layout_premium_mc4.py.
            for sheet_name, ws in writer.sheets.items():
                ...
                if sheet_name == "CAPA":
                    ...
                    ws.autofilter(0, 0, max(1, len(capa)), 1)
                elif sheet_name == "FONTES_ANALISADAS":
                    ...
                    ws.set_column(7, 8, 38)
                    ws.autofilter(0, 0, max(1, len(fontes)), 8)
                else:
                    ws.set_column(0, 0, 42)
                    ws.set_column(1, 7, 28)
                    ws.autofilter(0, 0, max(1, len(inconsistencias)), 7)
        aplicar_layout_premium_mc4_se_configurado(pre_path, pre_path, config, perfil="pre_validacao")
        if inconsistencias:
            print(f"Aviso: pre-validacao detectou {len(inconsistencias)} inconsistencias. Consulte {pre_path}.")
            if _is_sim(config.get("bloquear_pre_validacao", False)):
                raise ConfigError(f"Pre-validacao bloqueada devido a {len(inconsistencias)} inconsistencias.")



# ─── Escolha operacional de layout via tela/BAT ───────────────────────────────
def aplicar_layout_operacional_por_tela(config):
    """Aplica a escolha de layout recebida pela tela operacional/BAT.

    Prioridade:
    1. Variável de ambiente MC4_LAYOUT_NIVEL, definida pelo BAT com menu.
    2. config_arbitramento_rir70.json.
    3. padrão seguro: rapido.

    Esta função não altera regra fiscal nem cálculo. Apenas parametriza a
    apresentação final do Excel.
    """
    config = config or {}
    env_nivel = clean_str(os.environ.get("MC4_LAYOUT_NIVEL"))
    nivel = clean_str(env_nivel or config.get("layout_nivel") or "rapido").lower()
    ...
    nivel = aliases.get(nivel, nivel)
    if nivel not in {"rapido", "executivo", "auditoria"}:
        nivel = "rapido"

    config["layout_nivel"] = nivel
    config["_layout_nivel_origem"] = "Tela operacional" if env_nivel else "Config/default"

    if nivel == "auditoria":
        config["layout_usar_arquivo_raw_temporario"] = "Sim"
        config["layout_manter_raw_temporario"] = "Sim"
        config["layout_bloquear_se_falhar"] = "Sim"
        config["layout_validacao_profunda"] = "Sim"
        config["layout_aplicar_zebra"] = "Sim"
        config["layout_modo_rapido_acima_linhas"] = int(config.get("layout_modo_rapido_acima_linhas", 10000) or 10000)
    elif nivel == "executivo":
        config["layout_usar_arquivo_raw_temporario"] = "Nao"
        config["layout_manter_raw_temporario"] = "Nao"
        config["layout_bloquear_se_falhar"] = "Nao"
        config["layout_validacao_profunda"] = "Nao"
        config["layout_aplicar_zebra"] = "Sim"
        config["layout_modo_rapido_acima_linhas"] = int(config.get("layout_modo_rapido_acima_linhas", 10000) or 10000)
    else:
        config["layout_usar_arquivo_raw_temporario"] = "Nao"
        ...
        config["layout_bloquear_se_falhar"] = "Nao"
        config["layout_validacao_profunda"] = "Nao"
        config["layout_aplicar_zebra"] = "Nao"
        config["layout_modo_rapido_acima_linhas"] = int(config.get("layout_modo_rapido_acima_linhas", 10000) or 10000)

    config["fonte_excel"] = config.get("fonte_excel") or "Aptos Narrow"
    config["tamanho_fonte_excel"] = int(config.get("tamanho_fonte_excel", 10) or 10)

    print(f"Layout selecionado: {nivel.upper()} ({config['_layout_nivel_origem']})")
    return config
...
def main():
    ...
    config = aplicar_layout_operacional_por_tela(config)
    validar_governanca_operacional(config)
    config = validar_integridade_mc4(config)
    # Regenera o mapa de similaridade automaticamente antes de ler quaisquer inputs.
    _gerar_mapa_automatico(config)
    ensure_runtime_folders(config)
    # Executa pré‑validação dos arquivos de entrada. Caso ocorram inconsistências
    # críticas e a opção de bloqueio esteja habilitada, o motor será interrompido.
    _pre_validacao(config)
    ...
    validar_cnpjs_licenciados_mc4(records, config)
